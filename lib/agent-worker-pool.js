  ...
  try {
    if (process.connected) {
      process.send(msg, (err) => {
        // Ignore EPIPE errors which occur during shutdown
...
process.on('message', async (msg) => {
  try {
    if (msg.cmd === 'init') {
      ...
      logger.onEvent(evt => {
        if (['scenario', 'sent', 'received', 'tcp', 'fuzz', 'info'].includes(evt.type)) {
          ...
          safeSend({ type: 'logger', event: evt });
        }
      });
      ...
      if (!client) {
        safeSend({ type: 'result', result: { scenario: msg.scenarioName, status: 'ERROR', response: 'Client not initialized' } });
        safeSend({ type: 'ready' });
        return;
      }
      ...
      if (!scenario) {
        safeSend({ type: 'result', result: { scenario: msg.scenarioName, status: 'ERROR', response: 'Unknown scenario' } });
        safeSend({ type: 'ready' });
        return;
      }
      ...
      if (msg.baseline) {
        try {
          ...
          safeSend({ type: 'result', result: { scenario: msg.scenarioName, status: 'ERROR', response: err.message } });
        }
      } else {
        try {
          const result = await client.runScenario(scenario);
    ...
    safeSend({ type: 'logger', event: { type: 'info', message: `Worker error: ${err.message}` } });
    safeSend({ type: 'ready' });
  }
});
