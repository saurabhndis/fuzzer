...
function mergeNATStreams(streamList) {
  ...
  for (let i = 0; i < classified.length; i++) {
    ...
    for (let j = i + 1; j < classified.length; j++) {
      ...
      // Must share the same client port (NAT preserves port)
      if (a.effectiveClientPort !== b.effectiveClientPort) continue;
...
function analyzeFullHandshake(stream) {
  ...
  try {
    ...
    if (chRecord && clientParams) {
      clientParams.rawRecord = Buffer.from(chRecord.raw);
    }
