...
function readPcap(filePath) {
  ...
  while (offset + 16 <= buf.length) {
    ...
    // Transport Layer
    let srcPort, dstPort, payload, tcpFlags = 0;
    if (protocol === 6) { // TCP
      if (packet.length < pktOff + 20) continue;
...
function groupStreams(packets) {
  ...
  for (const p of packets) {
    ...
    const pktMeta = { direction: 'c2s', payload: p.payload, ts: p.ts, isFIN: p.isFIN || false, isRST: p.isRST || false, isSYN: p.isSYN || false };
...
function extractTcpControlActions(stream) {
  ...
  for (const pkt of stream.packets) {
    if (pkt.isRST) {
      const action = { type: 'rst', label: `TCP RST (from PCAP)` };
      if (pkt.direction === 'c2s') {
        clientActions.push(action);
      } else {
        serverActions.push(action);
      }
    } else if (pkt.isFIN && pkt.payload.length === 0) {
      // Only add FIN action for pure FIN packets (no payload).
      // FIN+data packets are handled by the normal send/recv flow.
      const action = { type: 'fin', label: `TCP FIN (from PCAP)` };
      if (pkt.direction === 'c2s') {
        clientActions.push(action);
      } else {
        serverActions.push(action);
...
function parsePcapToScenario(filePath, streamIndex = 0) {
  ...
  if (proto === 'tls') {
    ...
    const scenario = {
      ...
      category: 'Z',
      description: `Live TLS session recreated from PCAP.${stream.natMerged ? ' (NAT-merged)' : ''} ${tls13 ? 'TLS 1.3 with fresh key_share.' : (isECDHE && !clientAbortedBeforeCKE ? 'TLS 1.2 with live ECDHE key exchange.' : 'TLS 1.2 verbatim replay.')}`,
      side: 'client',
      protocol: 'tls',
      explanation: summary,
      pcapParams: {
