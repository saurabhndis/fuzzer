...
const { parseQuicRetry, buildQuicInitialWithCrypto, deriveServerInitialKeys, unprotectPacket, describeInitialPayload, MAX_CRYPTO_PER_INITIAL } = require('./quic-packet');
...
class QuicFuzzerClient {
  ...
  async runScenario(scenario) {
    ...
    let lastSentCryptoPayload = null; // Track the original CRYPTO payload for Retry handling
    let lastSentDcid = null;           // Track DCID for decrypting server Initial responses
    let lastSentQuicVersion = null;
    let localPort = null;
    ...
    try {
      ...
      localPort = this._allocateLocalPort ? this._allocateLocalPort() : 0;
      ...
      // Explicit bind before sending. Node binds implicitly on first send(),
      // but bind errors (EADDRINUSE, permission) then surface only as send
      // failures and can hang the scenario. Binding up-front lets us surface
      // them cleanly and await 'listening' for deterministic readiness.
      await new Promise((resolve, reject) => {
        const onErr = (err) => {
          if (this._releaseLocalPort && localPort) this._releaseLocalPort(localPort);
          reject(err);
        };
        this.socket.once('error', onErr);
        this.socket.bind(localPort || 0, isIPv6 ? '::' : '0.0.0.0', () => {
          this.socket.removeListener('error', onErr);
          const bound = this.socket.address();
          localPort = bound && bound.port ? bound.port : localPort;
          this.logger.info(`[quic] Bound local UDP port ${localPort}`);
          resolve();
        });
      });
        ...
        switch (action.type) {
          case 'send': {
            try {
              ...
              if (action._cryptoPayload) lastSentCryptoPayload = action._cryptoPayload;
              if (action._dcid) lastSentDcid = action._dcid;
              if (action._quicVersion) lastSentQuicVersion = action._quicVersion;
            } catch (e) {
          ...
          case 'recv': {
            ...
            let data = await this._waitForUDP(recvBuffer, recvTimeout);

            // Transparent QUIC Retry handling (RFC 9000 §8.1):
            // If the server sends a Retry, extract the token, rebuild the Initial
            // with the token and new DCID, resend, and wait for the real response.
            if (data && lastSentCryptoPayload) {
              const retry = parseQuicRetry(data);
              if (retry && retry.token.length > 0) {
                this.logger.info(`[quic] Received Retry — resending Initial with token (${retry.token.length}B)`);
                try {
                  // Fragment retry if payload exceeds single-packet capacity (e.g. PQC ClientHello)
                  if (lastSentCryptoPayload.length > MAX_CRYPTO_PER_INITIAL) {
                    let offset = 0;
                    let pn = 0;
                    while (offset < lastSentCryptoPayload.length) {
                      const chunk = lastSentCryptoPayload.slice(offset, offset + MAX_CRYPTO_PER_INITIAL);
                      const retryPacket = buildQuicInitialWithCrypto(chunk, {
                        dcid: retry.scid,
                        token: retry.token,
                        packetNumber: pn++,
                        cryptoOffset: offset,
                        version: lastSentQuicVersion || 0x00000001,
                      });
                      await this._sendUDP(retryPacket);
                      this.logger.sent(retryPacket, `[QUIC] Initial fragment (with Retry token) offset=${offset} (${chunk.length}B)`);
                      offset += MAX_CRYPTO_PER_INITIAL;
                    }
                  } else {
                    const retryPacket = buildQuicInitialWithCrypto(lastSentCryptoPayload, {
                      dcid: retry.scid,
                      token: retry.token,
                      packetNumber: 0,
                      version: lastSentQuicVersion || 0x00000001,
                    ...
                    await this._sendUDP(retryPacket);
                    this.logger.sent(retryPacket, '[QUIC] Initial (with Retry token)');
                  }
                  // Wait for the actual server response
                  data = await this._waitForUDP(recvBuffer, recvTimeout);
                } catch (e) {
                  this.logger.error(`Retry resend failed: ${e.message}`);
                }
              }
            }

            if (data) {
              lastResponse = this._describeQuicResponse(data, lastSentDcid, lastSentQuicVersion);
              rawResponse = data;
    ...
    if (['TIMEOUT', 'ERROR'].includes(status)) {
      ...
      // A live UDP endpoint that simply gives us nothing back is a silent
      // drop, not an infrastructure timeout. Preserve the response text, but
      // classify the outcome as DROPPED so distributed runs don't look flaky.
      if (status === 'TIMEOUT' && probe.udp.alive && expected !== 'TIMEOUT') {
        status = 'DROPPED';
        if (!lastResponse || /timeout/i.test(lastResponse)) {
          lastResponse = 'No UDP response (silent drop)';
        }
      }
    }
  ...
  _describeQuicResponse(data, dcid, quicVersion) {
    ...
    if (isLong) {
      ...
      // Try to decrypt Initial packets when we have the original DCID
      if (pktType === 0 && dcid) {
        const serverKeys = deriveServerInitialKeys(dcid, quicVersion || version);
        const result = unprotectPacket(data, serverKeys);
        if (result) {
          const findings = describeInitialPayload(result.payload);
          if (findings.length > 0) {
            return `QUIC ${typeName}: ${findings.join(', ')} (${data.length} bytes, v=0x${version.toString(16)})`;
          }
        }
        
        // If we failed to parse or decrypt the Initial packet, but it's a server response
        // to a malformed packet, it's likely a CONNECTION_CLOSE or drop.
        return `QUIC ${typeName} CONNECTION_CLOSE (Decryption failed or empty, ${data.length} bytes, v=0x${version.toString(16)})`;
      }

      if (pktType === 3) {
        return `QUIC Retry CONNECTION_CLOSE (Server rejected handshake, ${data.length} bytes, v=0x${version.toString(16)})`;
      }
