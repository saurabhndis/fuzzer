...
class QuicheServer {
  constructor(opts = {}) {
    ...
    this._pendingTimer = null;        // Timer for current scenario (cleared on new runScenario)
  }
  ...
  async runScenario(scenario) {
    ...
    if (this.pcapFileBase) {
      ...
      try {
        ...
        this.pcap = new PcapWriter(pcapFilename, { role: 'server', append: this.mergePcap, clientIP, serverIP, serverPort: this.port, clientPort: 49152 + Math.floor(Math.random() * 16000) });
      } catch (e) {
    ...
    // Cancel any pending scenario from a previous overlapping call
    if (this._pendingTimer) {
      clearTimeout(this._pendingTimer);
      this._pendingTimer = null;
    }

    // Clear stale state from previous iteration — the prior scenario's session
    // may not have closed yet, which would block new sessions from being handled.
    this.activeSessionHandler = null;
    this.activeSession = null;

    // Clear any dangling sessions from the queue that arrived late from a previous scenario
    while (this.sessionQueue.length > 0) {
      const stale = this.sessionQueue.shift();
