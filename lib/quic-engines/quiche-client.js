const fs = require('fs');
const dgram = require('dgram');
const { Logger } = require('../logger');
const { gradeResult } = require('../grader');
const { computeExpected } = require('../compute-expected');
const { QUIC_CATEGORY_SEVERITY } = require('../quic-scenarios');

class QuicheClient {
  constructor(opts = {}) {
    this.host = opts.host || 'localhost';
    this.port = opts.port || 443;
    this.timeout = opts.timeout || 5000;
    this.delay = opts.delay || 100;
    this.logger = opts.logger || new Logger(opts);
    this.quiche = opts.quicheLibrary;
    this.pcapFileBase = opts.pcapFile || null;
    this.mergePcap = opts.mergePcap || false;
    this.pcap = null;
    this.aborted = false;
    this._keylogFd = (opts.keylogFd !== undefined && opts.keylogFd !== null) ? opts.keylogFd : null;
    this._allocateLocalPort = opts.localPortAllocator || null;
    this._releaseLocalPort = opts.releaseLocalPort || null;
  }

  abort() {
    this.aborted = true;
    // Reject any pending Promise.race so the run unwinds immediately instead
    // of waiting for the connect/handshake timeout to expire. Without this,
    // /abort-scenario from the controller is a no-op for QuicheClient, and
    // every server-fuzz pair drags out to the full ~5s helper timeout.
    if (this._abortReject) {
      const reject = this._abortReject;
      this._abortReject = null;
      try { reject(new Error('Scenario aborted by peer-done')); } catch (_) {}
    }
  }

  async runScenario(scenario) {
    if (this.aborted) return { scenario: scenario.name, status: 'ABORTED', response: 'Aborted' };
    // Per-run abort handle: a Promise that rejects when abort() is called,
    // so any awaited Promise.race unwinds promptly.
    this._abortPromise = new Promise((_, reject) => { this._abortReject = reject; });
    // Swallow unhandled rejection if no race ever attaches.
    this._abortPromise.catch(() => {});

    this.logger.scenario(scenario.name, `${scenario.description} (quiche)`);

    // Initialize per-scenario PCAP if a base filename was provided
    if (this.pcapFileBase) {
      const { PcapWriter } = require('../pcap-writer');
      const path = require('path');
      const ext = path.extname(this.pcapFileBase) || '.pcap';
      const base = this.pcapFileBase.endsWith(ext)
        ? this.pcapFileBase.slice(0, -ext.length)
        : this.pcapFileBase;

      const pcapFilename = this.mergePcap
        ? this.pcapFileBase
        : `${base}.${scenario.name}.client${ext}`;

      try {
        const { clientIP, serverIP } = require('../pcap-writer').resolveIPs(this.host);

        this.pcap = new PcapWriter(pcapFilename, {
          role: 'client',
          append: this.mergePcap,
          clientIP,
          serverIP,
          clientPort: 49152 + Math.floor(Math.random() * 16000),
          serverPort: this.port,
        });
      } catch (e) {
        this.logger.error(`Failed to initialize PCAP: ${e.message}`);
        this.pcap = null;
      }
    }

    // Start UDP capture proxy when PCAP is enabled
    let udpProxy = null;
    let connectPort = this.port;
    if (this.pcap) {
      try {
        const proxyPort = this._allocateLocalPort ? this._allocateLocalPort() : 0;
        udpProxy = new UdpCaptureProxy({
          targetHost: this.host,
          targetPort: this.port,
          pcap: this.pcap,
          bindPort: proxyPort,
        });
        connectPort = await udpProxy.start();
        this.logger.info(`UDP capture proxy on port ${connectPort} -> ${this.host}:${this.port}`);
      } catch (e) {
        this.logger.error(`UDP proxy failed: ${e.message}, PCAP will be empty`);
        udpProxy = null;
        connectPort = this.port;
      }
    }

    let status = 'PASSED';
    let lastResponse = '';
    if (!this.pcap && this._allocateLocalPort) {
      this.logger.info('[quiche] client UDP source port is library-managed; explicit local port pinning is unavailable through the current http3 API');
    }

    try {
      let connectTimer;
      const connectTimeout = new Promise((_, reject) => {
        connectTimer = setTimeout(() => reject(new Error('QUIC connect/handshake timed out')), this.timeout);
      });
      const session = await Promise.race([this._connect(scenario, connectPort), connectTimeout, this._abortPromise]);

      // Ensure session is fully established (handshake complete)
      if (session.ready) {
        await Promise.race([session.ready(), connectTimeout, this._abortPromise]);
      }
      clearTimeout(connectTimer);

      if (scenario.clientHandler) {
        // Safety-net timeout to prevent infinite hangs if streams never complete
        const handlerTimeout = scenario.timeout || this.timeout;
        const makeTimeoutPromise = () => new Promise((_, reject) =>
          setTimeout(() => reject(new Error('Scenario handler timed out')), handlerTimeout)
        );

        // Custom handler (e.g. multi-stream scenarios)
        if (scenario.useNodeH2) {
          // If the scenario expects an H2 session (like h2-fw-*), pass the raw QUIC session
          // which provides the request() method for H3.
          const result = await Promise.race([
            scenario.clientHandler(session, this.host, this.logger),
            makeTimeoutPromise(),
          ]);
          status = result.status || 'PASSED';
          lastResponse = result.response || '';
        } else {
          // Wrap session in a shim that mimics a TLS socket for compatibility with firewall-scenarios
          // Use request() for H3 sessions, openStream() for raw QUIC
          const useH3 = typeof session.request === 'function';
          let activeStream = null;

          const newStream = () => {
            activeStream = useH3
              ? session.request({ ':method': 'POST', ':path': '/', ':scheme': 'https', ':authority': this.host })
              : session.openStream();
            shimWritten = false;
            shimEnded = false;
            return activeStream;
          };

          const getStream = () => {
            if (activeStream && !shimEnded) return activeStream;
            // Previous stream was ended (e.g. auto-end on 'data' listener) —
            // open a new stream to support multi-request handlers.
            return newStream();
          };

          let shimWritten = false;
          let shimEnded = false;
          // Track wrapped callbacks so removeListener works through the shim
          const listenerMap = new Map();
          // Buffer early events that arrive before listeners are attached
          let earlyData = [];
          let earlyResponse = null;
          let earlyEnd = false;
          let dataListenerAttached = false;

          // Parse HTTP/1.1 request line to extract method and path for H3 headers
          const parseHttpRequest = (data) => {
            const str = typeof data === 'string' ? data : data.toString();
            const match = str.match(/^(GET|POST|PUT|DELETE|HEAD|OPTIONS|PATCH)\s+(\S+)\s+HTTP/);
            if (match) return { method: match[1], path: match[2] };
            return null;
          };

          // Attach early buffering listeners to a newly created stream
          const attachEarlyBuffering = (stream) => {
            earlyData = [];
            earlyResponse = null;
            earlyEnd = false;
            dataListenerAttached = false;
            stream.on('response', (h) => {
              earlyResponse = h;
            });
            stream.on('data', (d) => {
              if (!dataListenerAttached) {
                earlyData.push(d);
              }
            });
            stream.on('end', () => {
              if (!dataListenerAttached) {
                earlyEnd = true;
              }
            });
          };

          const shim = {
            write: (data) => {
              // Only write() opens a new stream if the previous one was ended.
              // This supports multi-request handlers (write→read→write→read).
              if (useH3 && (shimEnded || !activeStream)) {
                // Parse HTTP/1.1 request to set proper H3 pseudo-headers
                const parsed = parseHttpRequest(data);
                const method = parsed ? parsed.method : 'POST';
                const path = parsed ? parsed.path : '/';
                activeStream = session.request({
                  ':method': method, ':path': path,
                  ':scheme': 'https', ':authority': this.host
                });
                shimWritten = false;
                shimEnded = false;
                attachEarlyBuffering(activeStream);
                // For GET/HEAD with no body, end the stream immediately
                if (parsed && (method === 'GET' || method === 'HEAD')) {
                  this.logger.sent(data, `HTTP/3 ${method} ${path}`);
                  shimWritten = true;
                  shimEnded = true;
                  activeStream.end();
                  return true;
                }
              } else if (!activeStream) {
                newStream();
                attachEarlyBuffering(activeStream);
              }
              this.logger.sent(data, `HTTP/3 Data (Stream)`);
              shimWritten = true;
              return activeStream.write(data);
            },
            on: (event, cb) => {
              // Event listeners always attach to the CURRENT stream, never open a new one.
              // This ensures 'data' and 'end' listeners stay on the same stream.
              const s = activeStream || getStream();
              if (event === 'data') {
                // The handler is done writing and waiting for response data.
                // In HTTP/3, the server won't respond until the request stream is
                // half-closed (ended on the write side).  TLS handlers rely on
                // Content-Length / Connection:close, but HTTP/3 needs an explicit end().
                if (shimWritten && !shimEnded) {
                  shimEnded = true;
                  try { s.end(); } catch (_) {}
                }
                let h3Status = earlyResponse ? (earlyResponse[':status'] || '200') : '';
                // Capture H3 response status to synthesize HTTP/1.1 headers
                s.on('response', (h) => { h3Status = h[':status'] || '200'; });
                let sentHeaders = false;
                const wrapped = (d) => {
                  this.logger.received(d, `HTTP/3 Data (Stream)`);
                  if (useH3 && !sentHeaders) {
                    // Synthesize HTTP/1.1 response so handlers that parse HTTP headers work
                    const status = h3Status || '200';
                    const header = `HTTP/1.1 ${status} OK\r\nContent-Length: ${d.length}\r\n\r\n`;
                    sentHeaders = true;
                    cb(Buffer.from(header + d.toString()));
                  } else {
                    cb(d);
                  }
                };
                listenerMap.set(cb, { stream: s, event: 'data', wrapped });
                dataListenerAttached = true;
                s.on('data', wrapped);
                // Replay any data that arrived before the listener was attached
                if (earlyData.length > 0) {
                  for (const d of earlyData) wrapped(d);
                  earlyData = [];
                }
              } else if (event === 'end') {
                const wrapped = (...args) => cb(...args);
                listenerMap.set(cb, { stream: s, event, wrapped });
                s.on(event, wrapped);
                // Replay early end
                if (earlyEnd) {
                  earlyEnd = false;
                  process.nextTick(() => cb());
                }
              } else {
                const wrapped = (...args) => cb(...args);
                listenerMap.set(cb, { stream: s, event, wrapped });
                s.on(event, wrapped);
              }
            },
            end: (data) => {
              const s = activeStream || getStream();
              if (data) this.logger.sent(data, `HTTP/3 Data (FIN)`);
              shimEnded = true;
              return s.end(data);
            },
            removeListener: (event, cb) => {
              const entry = listenerMap.get(cb);
              if (entry && entry.stream && entry.stream.removeListener) {
                entry.stream.removeListener(entry.event, entry.wrapped);
                listenerMap.delete(cb);
              } else if (activeStream && activeStream.removeListener) {
                activeStream.removeListener(event, cb);
              }
            },
            destroy: () => {
              if (!activeStream) return;
              if (activeStream.destroy) activeStream.destroy();
              else activeStream.close();
            },
            getProtocol: () => useH3 ? 'HTTP/3' : 'QUIC/Raw'
          };
          const result = await Promise.race([
            scenario.clientHandler(shim, this.host, this.logger, this.pcap, session),
            makeTimeoutPromise(),
          ]);

          // Ensure stream is ended if handler used it but didn't end it
          if (activeStream) {
            try { activeStream.end(); } catch (_) {}
          }

          status = result.status || 'PASSED';
          lastResponse = result.response || '';
        }
      } else {
        // Open one or more HTTP/3 streams, send requests, collect responses
        const streamCount = scenario.streamCount || 1;
        const useH3 = typeof session.request === 'function';
        const results = [];
        const authority = scenario.sni || this.host;
        const streamTimeout = scenario.timeout || this.timeout;

        for (let i = 0; i < streamCount; i++) {
          const resp = await new Promise((resolve) => {
            let buf = Buffer.alloc(0);
            let statusCode = '';

            if (useH3) {
              const req = session.request({
                ':method': 'GET',
                ':path': i === 0 ? '/' : `/stream-${i}`,
                ':scheme': 'https',
                ':authority': authority,
              });
              req.on('response', (h) => { statusCode = h[':status'] || ''; });
              req.on('data', (d) => {
                this.logger.received(d, `HTTP/3 Data (Stream ${i})`);
                buf = Buffer.concat([buf, d]);
              });
              req.on('end', () => resolve({ bytes: buf.length, status: statusCode }));
              req.on('error', () => resolve({ bytes: buf.length, status: statusCode }));
              req.end();
            } else {
              const stream = session.openStream();
              const payload = `GET / HTTP/1.1\r\nHost: ${authority}\r\nX-Stream: ${i}\r\n\r\n`;
              this.logger.sent(Buffer.from(payload), `QUIC Raw Data (Stream ${i})`);
              stream.end(payload);
              stream.on('data', (d) => {
                this.logger.received(d, `QUIC Raw Data (Stream ${i})`);
                buf = Buffer.concat([buf, d]);
              });
              stream.on('end', () => resolve({ bytes: buf.length, status: '' }));
              stream.on('error', () => resolve({ bytes: buf.length, status: '' }));
            }

            setTimeout(() => resolve({ bytes: buf.length, status: statusCode || '' }), streamTimeout);
          });

          results.push({ stream: i, ...resp });
        }

        const totalBytes = results.reduce((s, r) => s + r.bytes, 0);
        const got200 = results.some(r => String(r.status) === '200');
        if (got200) {
          lastResponse = `HTTP/3 200 OK, ${streamCount} stream(s), ${totalBytes} bytes total via quiche`;
        } else if (totalBytes > 0) {
          const statuses = results.map(r => r.status || 'no-status').join(', ');
          lastResponse = `HTTP/3 ${streamCount} stream(s), ${totalBytes} bytes total via quiche (status: ${statuses})`;
          status = 'DROPPED';
        } else {
          lastResponse = 'No stream data received';
          status = 'DROPPED';
        }
      }

      // Allow final data to be processed before closing
      await new Promise(r => setTimeout(r, 200));
      try { session.close(); } catch (_) {}
    } catch (e) {
      this.logger.error(`[quiche] ${e.message}`);
      // Distinguish controller-initiated peer-done from real failures so the
      // verdict for helper-side scenarios doesn't get tagged as ERROR or
      // confused with a handshake timeout.
      if (this.aborted || /scenario aborted/i.test(e.message)) {
        status = 'PEER_DONE';
        lastResponse = e.message;
      } else if (/ECONNREFUSED|timeout|timed out|closed|reset/i.test(e.message)) {
        // Connection refused / timeout = server dropped the connection (expected for fuzz targets)
        status = 'DROPPED';
        lastResponse = e.message;
      } else {
        status = 'ERROR';
        lastResponse = e.message;
      }
    } finally {
      // Drop per-run abort handle so the next run starts with a fresh one.
      this._abortReject = null;
      this._abortPromise = null;
      // Allow proxy to capture any final packets before closing
      if (udpProxy) {
        await new Promise(r => setTimeout(r, 100));
        udpProxy.stop();
      }
      if (this.pcap) {
        this.pcap.close();
        this.pcap = null;
      }
    }

    const computed = computeExpected(scenario);
    const expected = scenario.expected || computed.expected;
    const expectedReason = scenario.expectedReason || computed.reason;
    const verdict = this._computeVerdict(status, expected, lastResponse);
    const severity = QUIC_CATEGORY_SEVERITY[scenario.category] || 'medium';

    const result = {
      scenario: scenario.name, description: scenario.description,
      category: scenario.category, severity,
      status, expected, verdict,
      response: lastResponse || status,
      compliance: null, finding: null, hostDown: false, probe: null,
    };
    result.finding = gradeResult(result, scenario);
    this.logger.result(scenario.name, status, lastResponse || 'No response', verdict, expectedReason, false, result.finding, null);
    return result;
  }

  _connect(scenario, connectPort) {
    const port = connectPort || this.port;
    return new Promise((resolve, reject) => {
      const timer = setTimeout(() => {
        reject(new Error('QUIC connection timed out'));
      }, this.timeout);

      try {
        // Use HTTP/3 API (lib.connect) for proper ALPN and H3 framing;
        // fall back to raw QUIC (connectQuic) for non-H3 scenarios.
        const connectFn = this.quiche.connect || this.quiche.connectQuic;
        // Connect to localhost only when using the UDP capture proxy (proxy changes the port)
        const connectHost = (connectPort && connectPort !== this.port) ? 'localhost' : this.host;
        const target = this.quiche.connect
          ? `https://${connectHost}:${port}`
          : `${connectHost}:${port}`;

        const session = connectFn(target, {
          rejectUnauthorized: false,
          ciphers: scenario.ciphers,
          curves: scenario.curves,
          alpn: scenario.alpn,
          servername: scenario.sni || this.host,
          keylog: !!this._keylogFd,
          initialMaxStreamsBidi: 10000,
          initialMaxData: 100_000_000,
          initialMaxStreamDataBidiLocal: 10_000_000,
        });

        if (this._keylogFd && session.on) {
          const fd = this._keylogFd;
          session.on('keylog', (line) => {
            try { fs.writeSync(fd, line); } catch (_) {}
          });
        }

        session.on('connect', () => {
          clearTimeout(timer);
          resolve(session);
        });

        session.on('error', (e) => {
          clearTimeout(timer);
          reject(e);
        });
      } catch (e) {
        clearTimeout(timer);
        reject(e);
      }
    });
  }

  _computeVerdict(status, expected, response) {
    if (!expected || status === 'ERROR' || status === 'ABORTED') return 'N/A';
    if (response && /QUIC.*CONNECTION_CLOSE/i.test(response)) return 'AS EXPECTED';
    if (response && /Version Negotiation/i.test(response)) return 'AS EXPECTED';
    const effective = status === 'TIMEOUT' ? 'DROPPED' : status;
    const expectedEffective = expected === 'TIMEOUT' ? 'DROPPED' : expected;
    return effective === expectedEffective ? 'AS EXPECTED' : 'UNEXPECTED';
  }
}

// ─── UDP Capture Proxy ─────────────────────────────────────────────────────
// Transparent UDP proxy that captures all QUIC packets flowing between the
// quiche client and the real server, writing them to a PcapWriter.
// Same pattern as TcpCaptureProxy used for HTTP/2.
class UdpCaptureProxy {
  constructor({ targetHost, targetPort, pcap, bindPort = 0 }) {
    this.targetHost = targetHost;
    this.targetPort = targetPort;
    this.pcap = pcap;
    this.bindPort = bindPort || 0;
    this.socket = null;
    this._clientAddr = null;
    this._clientPort = null;
  }

  start() {
    return new Promise((resolve, reject) => {
      this.socket = dgram.createSocket('udp4');

      this.socket.on('message', (msg, rinfo) => {
        if (!this._clientAddr) {
          // First packet comes from quiche client — remember its address
          this._clientAddr = rinfo.address;
          this._clientPort = rinfo.port;
        }

        if (rinfo.port === this._clientPort && rinfo.address === this._clientAddr) {
          // Client -> Server: forward to real target and capture as 'sent'
          try { this.pcap.writeUDPPacket(msg, 'sent'); } catch (_) {}
          this.socket.send(msg, this.targetPort, this.targetHost);
        } else {
          // Server -> Client: forward back to quiche client and capture as 'received'
          try { this.pcap.writeUDPPacket(msg, 'received'); } catch (_) {}
          if (this._clientAddr && this._clientPort) {
            this.socket.send(msg, this._clientPort, this._clientAddr);
          }
        }
      });

      this.socket.on('error', (e) => reject(e));

      // Bind to random available port on localhost
      this.socket.bind(this.bindPort, 'localhost', () => {
        try { this.socket.setRecvBufferSize(8*1024*1024); this.socket.setSendBufferSize(8*1024*1024); } catch(e){}
        const addr = this.socket.address();
        resolve(addr.port);
      });
    });
  }

  stop() {
    if (this.socket) {
      try { this.socket.close(); } catch (_) {}
      this.socket = null;
    }
  }
}

module.exports = { QuicheClient };
