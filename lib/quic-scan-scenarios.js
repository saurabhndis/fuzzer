  ...
  for (const qv of QUIC_VERSIONS) {
    for (const cs of TLS13_CIPHERS) {
      ...
      for (const group of SCAN_GROUPS) {
        ...
        // For valid combinations on QUICv1, use the full quiche state machine
        // to verify that a complete handshake and data exchange succeeds.
        if (qv.id === 0x00000001 && group.keySize > 0) {
          scenario.useQuiche = true;
          scenario.ciphers = csName;
          // Map scan names to OpenSSL/quiche names for groups
          const groupMap = { 'P-256': 'P-256', 'P-384': 'P-384', 'P-521': 'P-521', 'X25519': 'X25519', 'MLKEM768': 'MLKEM768', 'X25519_MLKEM768': 'X25519_MLKEM768' };
          scenario.curves = groupMap[group.name] || group.name;
        }
