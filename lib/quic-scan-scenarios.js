...
const SCAN_GROUPS = [
  ...
  { id: NamedGroup.MLKEM1024,           name: 'MLKEM1024',          keySize: 1568 },
  ...
  // FrodoKEM and Classic McEliece removed — not supported by quiche/BoringSSL
];
...
function buildQuicScanInitial(quicVersion, clientHelloBody, opts = {}) {
  ...
  // Long header: PN length encoded in low 2 bits, packet type in bits 4-5
  // QUICv2 (RFC 9369) remaps Initial to type 0b01; QUICv1 uses 0b00
  const packetType = (quicVersion === 0x6b3343cf) ? 1 : 0;
...
function generateQuicScanScenarios() {
  ...
  for (const qv of QUIC_VERSIONS) {
    for (const cs of TLS13_CIPHERS) {
      ...
      for (const group of SCAN_GROUPS) {
        ...
        // For valid combinations on QUICv1 with standard groups, use the full
        // quiche state machine to verify a complete handshake + data exchange.
        // PQC groups (MLKEM, FrodoKEM, McEliece, hybrids) are not supported by
        // the quiche native library — those fall through to the raw QUIC Initial
        // path which crafts proper ClientHellos with PQC key shares.
        const QUICHE_SUPPORTED_GROUPS = new Set(['P-256', 'P-384', 'P-521', 'X25519']);
        if (qv.id === 0x00000001 && QUICHE_SUPPORTED_GROUPS.has(group.name)) {
          scenario.useQuiche = true;
        } else {
          scenario.expected = 'DROPPED';
        }
        ...
        scenario.actions = (opts) => {
          ...
          if (group.keySize > 0 && group.keySize <= 65535) {
            extraExtensions.push({
          ...
          return [
            { type: 'send', data: buildQuicScanInitial(qv.id, chBody, { dcid, scid }), label: `QUIC Scan: ${qv.name} | ${csName} | ${group.name}`, _dcid: dcid, _quicVersion: qv.id },
            { type: 'recv', timeout: 5000 },
          ];
        };

        QUIC_SCAN_SCENARIOS.push(scenario);
      }
    }
  }
  ...
  for (const alpn of ALPN_PROTOCOLS) {
    ...
    scenario.actions = (opts) => {
      ...
      return [
        { type: 'send', data: buildQuicScanInitial(0x00000001, chBody, { dcid, scid }), label: `QUIC Scan: ALPN ${alpn}`, _dcid: dcid, _quicVersion: 0x00000001 },
        { type: 'recv', timeout: 5000 },
      ];
    };
  ...
  QUIC_SCAN_SCENARIOS.push({
    ...
    expected: 'DROPPED',
    expectedReason: 'A Version Negotiation response is useful when present; a silent drop is also acceptable for this unsupported-version probe.',
    });

    // 4. Grease Extensions Scan — probe if server tolerates GREASE (RFC 8701)
    QUIC_SCAN_SCENARIOS.push({
    name: 'qscan-grease-extensions',
    category: 'QSCAN',
    description: 'QUIC scan: GREASE (RFC 8701) tolerance — sends random unknown extensions',
    side: 'client',
    actions: (opts) => {
    const hostname = opts.hostname || 'localhost';
    const dcid = crypto.randomBytes(8);
    ...
    const extraExtensions = [
      { type: ExtensionType.SUPPORTED_VERSIONS, data: Buffer.from([0x02, 0x03, 0x04]) },
      { type: 0x1a1a, data: crypto.randomBytes(4) }, // GREASE
      { type: 0x39, data: buildScanTransportParams(scid) }, // Transport Params
    ];
    ...
    return [
      { type: 'send', data: buildQuicScanInitial(0x00000001, chBody, { dcid, scid }), label: 'QUIC Scan: GREASE Extensions', _dcid: dcid, _quicVersion: 0x00000001 },
      { type: 'recv', timeout: 5000 },
    ];
    },
    expected: 'PASSED',
    ...
    // 5. Signature Algorithms Scan — probe supported signature schemes
    const SIG_SCHEMES = [
      { id: SignatureScheme.ECDSA_SECP256R1_SHA256, name: 'ecdsa-p256' },
      { id: SignatureScheme.RSA_PSS_RSAE_SHA256,     name: 'rsa-pss-sha256' },
      { id: SignatureScheme.ED25519,                 name: 'ed25519' },
    ];

    for (const sig of SIG_SCHEMES) {
      QUIC_SCAN_SCENARIOS.push({
        name: `qscan-sigalg-${sig.name}`,
        category: 'QSCAN',
        description: `QUIC scan: Signature Algorithm ${sig.name} support`,
        side: 'client',
        actions: (opts) => {
          const hostname = opts.hostname || 'localhost';
          const dcid = crypto.randomBytes(8);
          ...
          const extraExtensions = [
            { type: ExtensionType.SUPPORTED_VERSIONS, data: Buffer.from([0x02, 0x03, 0x04]) },
            {
              ...
              data: (() => {
                ...
                b.writeUInt16BE(2, 0);
                b.writeUInt16BE(sig.id, 2);
                return b;
              })(),
            },
            { type: 0x39, data: buildScanTransportParams(scid) },
          ];
          ...
          return [
            { type: 'send', data: buildQuicScanInitial(0x00000001, chBody, { dcid, scid }), label: `QUIC Scan: SigAlg ${sig.name}`, _dcid: dcid, _quicVersion: 0x00000001 },
            { type: 'recv', timeout: 5000 },
          ];
        },
        expected: 'PASSED',
