...
class SSHDeployer extends EventEmitter {
  ...
  constructor(opts) {
    ...
    // customStartCmd: override the default `node client.js --agent ...` command.
    // Use {{controlPort}} and {{token}} as substitution placeholders.
    this.customStartCmd = opts.customStartCmd || null;
    // skipReadyCheck: skip _verifyProtocolReadiness and _waitForReady.
    // Use when the remote process has no HTTP control API (e.g. cert-verify-server).
    this.skipReadyCheck = opts.skipReadyCheck || false;
    this._isRoot = false;
  ...
  async deploy(bundleBuffer) {
    ...
    if (this.skipReadyCheck) {
      // No HTTP control API — wait briefly for the process to start
      await new Promise(r => setTimeout(r, 3000));
      this._log('ready', 'Agent process started (skipping readiness check)');
    } else {
  ...
  async _startAgent() {
    let cmd;
    if (this.customStartCmd) {
      cmd = this.customStartCmd
        .replace(/\{\{controlPort\}\}/g, String(this.controlPort))
        .replace(/\{\{token\}\}/g, this.token);
    } else {
      const entryFile = this.role === 'client' ? 'client.js' : 'server.js';
      cmd = `node ${entryFile} --agent --control-port ${this.controlPort} --token ${this.token}`;
    }

    if (this.isWindows) {
      await this._exec(
        `cd "%USERPROFILE%\\${REMOTE_DIR}" && start /B ${cmd} > agent.log 2>&1`
      );
    } else {
      await this._exec(
        `cd ~/${REMOTE_DIR} && nohup ${cmd} > agent.log 2>&1 & disown`
      );
    }
  }
