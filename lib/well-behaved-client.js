...
class WellBehavedClient {
  ...
  // Raw H2 byte-reader. Sends the H2 connection preface, SETTINGS, and a single GET
  // HEADERS frame, then drains whatever the server sends. Used for server-side
  // scenarios that emit deliberately malformed H2 frames (CONTINUATION floods, bogus
  // :status, etc.). Bypasses Node's http2 module so we don't trigger the native
  // ~Http2Session(current_nghttp2_memory_ == 0) assertion on parser errors.
  connectRawH2() {
    const PREFACE = Buffer.from('PRI * HTTP/2.0\r\n\r\nSM\r\n\r\n');
    // SETTINGS frame: length=0, type=4, flags=0, streamId=0
    const SETTINGS = Buffer.from([0,0,0, 4, 0, 0,0,0,0]);
    // HEADERS frame for GET /: HPACK indexed :method=GET (0x82), :scheme=http (0x86),
    // :path=/ (0x84), :authority=localhost (literal incremental). Keep it minimal.
    // Use a simple fully-indexed GET / on stream 1 with END_STREAM | END_HEADERS.
    const HDR_PAYLOAD = Buffer.from([0x82, 0x86, 0x84, 0x01]); // :method GET, :scheme http, :path /, :authority dynamic-empty (illegal but harmless here; server only needs HEADERS)
    const HEADERS = Buffer.concat([
      Buffer.from([0, 0, HDR_PAYLOAD.length, 1, 0x05, 0,0,0,1]), // type=HEADERS, flags=END_STREAM|END_HEADERS, stream=1
      HDR_PAYLOAD,
    ]);
    ...
    const attempt = (retryCount) => {
      return new Promise((resolve) => {
        ...
        socket.once('connect', () => {
          if (this.logger) this.logger.info('[local-client] Raw H2 connected');
          try { socket.write(PREFACE); socket.write(SETTINGS); socket.write(HEADERS); } catch (_) {}
        });
        socket.on('data', () => {}); // drain
        socket.once('error', (err) => {
          if ((err.code === 'ECONNREFUSED' || (err.message || '').includes('ECONNREFUSED')) && retryCount < maxRetries && !this._stopped) {
            if (this.logger) this.logger.info(`[local-client] Raw H2 connection refused, retrying (${retryCount + 1}/${maxRetries})...`);
            try { socket.destroy(); } catch (_) {}
            this.activeSockets.delete(socket);
            setTimeout(() => attempt(retryCount + 1).then(resolve), retryDelay);
            return;
          }
          if (this.logger) this.logger.info(`[local-client] Raw H2 socket error (expected): ${err.message}`);
          settle();
        });
        socket.once('close', () => settle());
        socket.setTimeout(10000, () => settle());
      });
    };
    return attempt(0);
  }
  ...
  connectH2() {
    ...
    const attempt = (retryCount) => {
      return new Promise((resolve) => {
        ...
        let session;
        try {
          session = http2.connect(`http://${this.host}:${this.port}`, {
          });
        } catch (e) {
        ...
        session.on('error', (err) => {
          if ((err.code === 'ECONNREFUSED' || err.message.includes('ECONNREFUSED')) && retryCount < maxRetries && !this._stopped) {
            retrying = true;
            if (this.logger) this.logger.info(`[local-client] H2 connection refused, retrying (${retryCount + 1}/${maxRetries})...`);
            // Free the dead session before retrying so it doesn't get GC'd while
            // nghttp2 still has outstanding memory (would trigger ~Http2Session assertion).
            if (this._connection === session) this._connection = null;
            teardown(session);
            setTimeout(() => attempt(retryCount + 1).then(resolve), retryDelay);
            return;
          }
        ...
        session.setTimeout(10000, () => {
          if (!retrying) { teardown(session); resolve(); }
        });
      });
    };
  ...
  stop() {
    this._stopped = true;
    if (this._connection) {
      // Order matters: close() flushes nghttp2 state; destroy() then frees C++ memory
      // before GC sees the session. Reversing the order leaves nghttp2 buffers behind
      // and trips the ~Http2Session assertion (current_nghttp2_memory_ == 0).
      try { this._connection.close(); } catch (_) {}
      try { this._connection.destroy(); } catch (_) {}
      this._connection = null;
    }
    for (const socket of this.activeSockets) {
      try { if (typeof socket.close === 'function') socket.close(); } catch (_) {}
      try { socket.destroy(); } catch (_) {}
    }
