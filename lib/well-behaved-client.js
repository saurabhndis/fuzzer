...
class WellBehavedClient {
  ...
  /**
   * Connect via raw TCP and send a synthetic ClientHello — for server-side TLS fuzz scenarios
   * that operate on raw TCP sockets (not Node's TLS library).
   */
  connectRawTLS() {
    return new Promise((resolve) => {
      ...
      socket.on('connect', () => {
        if (this.logger) this.logger.info('[local-client] Raw TCP connected');
        // Send a synthetic ClientHello as a TLS record
        const ch = buildSyntheticClientHello();
        const record = Buffer.alloc(5 + ch.length);
        record[0] = 0x16; // Handshake
        record[1] = 0x03; record[2] = 0x01; // TLS 1.0
        record.writeUInt16BE(ch.length, 3);
        ch.copy(record, 5);
        try { socket.write(record); } catch (_) {}
      });

      socket.on('data', () => {});
      socket.on('error', (err) => {
        if (this.logger) this.logger.info(`[local-client] Raw TCP error (expected): ${err.message}`);
        resolve();
      });
