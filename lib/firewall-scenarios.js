...
const { classifyThreatTransfer, parseHttp1Response } = require('./threat-transfer-verdict');
...
function generateTLSFirewallScenarios() {
  return PAYLOADS.map(payload => ({
    ...
    clientHandler: async (socket, host, logger) => {
      ...
      const parsed = parseHttp1Response(resp.buf);
      return classifyThreatTransfer({
        protocol: 'TLS',
        family: 'virus/threat',
    ...
    expected: 'PASSED',
    expectedReason: 'Pass if the payload downloads successfully; also pass if the firewall blocks, resets, or replaces the threat after the TLS handshake.',
  }));
}
...
function generateH2FirewallScenarios() {
  return PAYLOADS.map(payload => ({
    ...
    expected: 'PASSED',
    expectedReason: 'Pass if the payload downloads successfully; also pass if the firewall blocks, resets, or replaces the threat after the HTTP/2 handshake.',
  }));
}
...
const FW_SRV_SCENARIOS = [
  {
    ...
    expected: 'PASSED',
    expectedReason: 'Pass if virus downloads are delivered; also pass if a firewall blocks or replaces them after handshake.',
  },
  {
...
FW_SRV_SCENARIOS.push({
  ...
  serverHandler: async (rinfo, sendFn, log, clientPacket) => {
    ...
    return {
      status: 'PASSED',
      response: `Sent ${VIRUS_PAYLOADS.length} virus payloads in one QUIC datagram; pass if delivered or blocked/replaced by FW`,
    };
  },
  expected: 'PASSED',
  expectedReason: 'Pass if virus downloads are delivered; also pass if a firewall blocks or replaces them after handshake.',
...
FW_SRV_SCENARIOS.push({
  ...
  serverHandler: async (rinfo, sendFn, log, clientPacket) => {
    ...
    return {
      status: 'PASSED',
      response: `Sent ${VIRUS_PAYLOADS.length} virus payloads in separate QUIC datagrams; pass if delivered or blocked/replaced by FW`,
    };
  },
  expected: 'PASSED',
  expectedReason: 'Pass if virus downloads are delivered; also pass if a firewall blocks or replaces them after handshake.',
...
function generateQuicFirewallScenarios() {
  return PAYLOADS.map(payload => ({
    ...
    expected: 'PASSED',
    expectedReason: 'Pass if the payload downloads successfully; also pass if the firewall blocks, resets, or replaces the threat after the HTTP/3 handshake.',
  }));
}
