  ...
  return PAYLOADS.map(payload => ({
    ...
    clientHandler: async (socket, host, logger) => {
      ...
      const resp = await new Promise((resolve) => {
        ...
        let outcome = 'timeout';
        let errMsg = '';
        const settle = () => resolve({ buf, outcome, errMsg });
        socket.on('data', (d) => { buf = Buffer.concat([buf, d]); });
        socket.on('end', () => { outcome = 'end'; settle(); });
        socket.on('error', (e) => { outcome = 'error'; errMsg = e && e.message || String(e); settle(); });
        setTimeout(() => settle(), 5000);
      });

      const respStr = resp.buf.toString('utf8', 0, Math.min(resp.buf.length, 500));
      logger.info(`[fw] ${payload.id}: sent ${data.length}B, received ${resp.buf.length}B (${resp.outcome}${resp.errMsg ? `: ${resp.errMsg}` : ''})`);

      if (resp.buf.length === 0) {
        if (resp.outcome === 'error') {
          return { status: 'DROPPED', response: `Connection error before response — ${resp.errMsg || 'reset'} (cause unknown: firewall, peer, or transport)` };
        }
        if (resp.outcome === 'end') {
          return { status: 'DROPPED', response: `Peer closed connection without sending a response (cause unknown: firewall, peer, or transport)` };
        }
        return { status: 'TIMEOUT', response: `No response within 5s — inconclusive (could be firewall, slow peer, or transport issue)` };
      }
      ...
      return { status: 'PASSED', response: `Not blocked — ${resp.buf.length}B response (firewall did not detect ${payload.name})` };
    },
    expected: 'DROPPED',
  ...
  return PAYLOADS.map(payload => ({
    ...
    clientHandler: async (session, host, logger) => {
      ...
      const result = await new Promise((resolve) => {
        ...
        let outcome = 'timeout';
        let errMsg = '';
        const settle = () => resolve({ status: respStatus, data: respData, outcome, error: errMsg });
        req.on('response', (h) => { respStatus = h[':status']; });
        req.on('data', (d) => { respData = Buffer.concat([respData, d]); });
        req.on('end', () => { outcome = 'end'; settle(); });
        req.on('error', (e) => { outcome = 'error'; errMsg = e && e.message || String(e); settle(); });
        setTimeout(() => settle(), 5000);
      });

      logger.info(`[h2-fw] ${payload.id}: sent ${data.length}B, status=${result.status} resp=${result.data.length}B (${result.outcome}${result.error ? `: ${result.error}` : ''})`);

      if (result.status === 0) {
        if (result.outcome === 'error') {
          return { status: 'DROPPED', response: `Stream error before response — ${result.error || 'reset'} (cause unknown: firewall, peer, or transport)` };
        }
        if (result.outcome === 'end') {
          return { status: 'DROPPED', response: `Stream closed without :status (cause unknown: firewall, peer, or transport)` };
        }
        return { status: 'TIMEOUT', response: `No HTTP/2 response within 5s — inconclusive (could be firewall, slow peer, or transport issue)` };
      }
...
FW_SRV_SCENARIOS.push({
  name: 'srv-quic-h3-multi-virus-sequential',
  category: 'QS',
  description: 'Server sends all virus files sequentially in one HTTP/3 stream',
  side: 'server',
  useQuiche: true,
  h3Handler: async (stream, headers, logger) => {
    logger.info(`Sending ${VIRUS_PAYLOADS.length} virus files sequentially over one HTTP/3 stream...`);
    stream.respond({ ':status': '200', 'content-type': 'text/plain' });
    for (const p of VIRUS_PAYLOADS) {
      const data = typeof p.data === 'string' ? Buffer.from(p.data) : p.data;
      stream.write(`--- ${p.id} ---\n`);
      stream.write(data);
      stream.write('\n');
      logger.sent(data, `H3 Virus Payload: ${p.id}`);
      await new Promise(r => setTimeout(r, 50)); // small delay to help firewall inspection
    }
    stream.end();
  },
  expected: 'DROPPED',
  expectedReason: 'Firewall should detect virus files in the HTTP/3 server response and drop the connection',
});
...
function generateQuicFirewallScenarios() {
  return PAYLOADS.map(payload => ({
    name: `quic-fw-${payload.id}`,
    category: 'QM',
    description: `QUIC Firewall: ${payload.name} — ${payload.description}`,
    side: 'client',
    protocol: 'quic',
    useQuiche: true,
    clientHandler: async (shim, host, logger, pcap, session) => {
      ...
      logger.info(`[quic-fw] ${payload.id}: sent ${data.length}B, status=${result.status} resp=${result.data.length}B (${result.outcome}${result.error ? `: ${result.error}` : ''})`);

      if (result.status === 0) {
        if (result.outcome === 'error') {
          return { status: 'DROPPED', response: `H3 stream error before response — ${result.error || 'reset'} (cause unknown: firewall, peer, or transport)` };
        }
        if (result.outcome === 'end') {
          return { status: 'DROPPED', response: `H3 stream closed without :status (cause unknown: firewall, peer, or transport)` };
        }
        return { status: 'TIMEOUT', response: `No HTTP/3 response within 5s — inconclusive (could be firewall, slow peer, or transport issue)` };
      }
      if (result.status === 403 || result.status === 406 || result.status === 451) {
        return { status: 'DROPPED', response: `Blocked (HTTP ${result.status}) — firewall rejected` };
      }
    ...
    expected: 'DROPPED',
    expectedReason: `Firewall/IPS should detect and block over QUIC/HTTP3: ${payload.description}`,
  }));
}

const FW_TLS_SCENARIOS = generateTLSFirewallScenarios();
const FW_H2_SCENARIOS = generateH2FirewallScenarios();
const FW_QUIC_SCENARIOS = generateQuicFirewallScenarios();
