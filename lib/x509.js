...
function derInteger(value) {
  ...
  // Buffer input — ensure proper DER encoding for positive integers
  if (Buffer.isBuffer(value)) {
    // Strip any leading zero bytes to normalize
    let start = 0;
    while (start < value.length - 1 && value[start] === 0) start++;
    const trimmed = value.slice(start);
    // Add leading zero if high bit is set (ASN1 integers are signed)
    if (trimmed[0] & 0x80) {
      return derEncode(TAG.INTEGER, Buffer.concat([Buffer.from([0x00]), trimmed]));
    }
    return derEncode(TAG.INTEGER, trimmed);
  }
