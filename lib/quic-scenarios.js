...
const { buildQuicInitialWithCrypto, buildQuicConnectionClose, encodeVarInt, deriveInitialKeys, protectPacket, MAX_CRYPTO_PER_INITIAL } = require('./quic-packet');
...
const QUIC_SCENARIOS = [
...

  {
    ...
    side: 'client',
    useQuiche: true, // Use native quiche engine
    streamCount: 1,
    expected: 'PASSED',
  },
...

  {
    ...
    side: 'client',
    useQuiche: true,
    clientHandler: async (shim, host, logger, pcap, session) => {
  ...
  {
    name: 'quic-well-behaved-max-streams-get-100kb',
    category: 'QZ',
    description: 'HTTP/3 GET: 100 concurrent streams each downloading 100KB (max bidi stream limit)',
    side: 'client',
    useQuiche: true,
    timeout: 120000,
    clientHandler: async (shim, host, logger, pcap, session) => {
      ...
      for (let batch = 0; batch < STREAM_COUNT; batch += BATCH_SIZE) {
        const batchEnd = Math.min(batch + BATCH_SIZE, STREAM_COUNT);
        const requests = [];
        for (let i = batch; i < batchEnd; i++) {
          const req = session.request({
            ':method': 'GET',
            ':path': `/data?size=${PAYLOAD_SIZE}&stream=${i}`,
            ':scheme': 'https',
            ':authority': host,
          });
          req.end();
          requests.push(new Promise((resolve) => {
            ...
            req.on('end', () => { completed++; resolve(bytes); });
      ...
      if (errors === STREAM_COUNT) return { status: 'DROPPED', response: `All ${STREAM_COUNT} GET streams failed` };
      return {
        status: 'PASSED',
        response: `${completed}/${STREAM_COUNT} GET streams OK, ${totalBytes} bytes total (${totalBytes / 1024}KB)${errors > 0 ? `, ${errors} errors` : ''}`,
      };
    },
    expected: 'PASSED',
  },
  ...
  {
    name: 'quic-well-behaved-max-streams-post-100kb',
    category: 'QZ',
    description: 'HTTP/3 POST: 100 concurrent streams each uploading and echo-verifying 100KB (max bidi stream limit)',
    side: 'client',
    useQuiche: true,
    timeout: 120000,
    clientHandler: async (shim, host, logger, pcap, session) => {
      ...
      const requests = payloads.map((payload, i) => {
        const req = session.request({
          ':method': 'POST',
          ':path': `/echo?stream=${i}`,
          ':scheme': 'https',
      ...
      await Promise.all(requests);
      if (errors === STREAM_COUNT) return { status: 'DROPPED', response: `All ${STREAM_COUNT} POST streams failed` };
      const totalBytes = STREAM_COUNT * PAYLOAD_SIZE * 2; // uploaded + echoed back
      return {
        status: 'PASSED',
        response: `${verified}/${STREAM_COUNT} POST streams echo-verified, ${totalBytes / 1024}KB exchanged`,
      };
    },
    expected: 'PASSED',
  },
  ...
  {
    name: 'quic-well-behaved-max-streams-delete-100kb',
    category: 'QZ',
    description: 'HTTP/3 DELETE: 100 concurrent streams each receiving a 100KB response (max bidi stream limit)',
    side: 'client',
    useQuiche: true,
    timeout: 120000,
    clientHandler: async (shim, host, logger, pcap, session) => {
      ...
      for (let batch = 0; batch < STREAM_COUNT; batch += BATCH_SIZE) {
        ...
        for (let i = batch; i < batchEnd; i++) {
          const req = session.request({
            ':method': 'DELETE',
            ':path': `/resource/${i}?size=${PAYLOAD_SIZE}`,
            ':scheme': 'https',
            ':authority': host,
          });
          req.end();
      ...
      if (errors === STREAM_COUNT) return { status: 'DROPPED', response: `All ${STREAM_COUNT} DELETE streams failed` };
      return {
        status: 'PASSED',
        response: `${completed}/${STREAM_COUNT} DELETE streams OK, ${totalBytes / 1024}KB received${errors > 0 ? `, ${errors} errors` : ''}`,
      };
    },
    expected: 'PASSED',
  },

  {
    ...
    side: 'server',
    useQuiche: true, // Run via QuicheServer
    timeout: 500,           // Short timeout to avoid long hangs on blocked client tests
    expected: 'PASSED',
  },
...

  {
    ...
    serverHandler: async (rinfo, sendFn, log, msg) => {
      ...
      const hdrFirstByte = Buffer.from([0x80 | 0x40 | (packetType << 4) | (pnLen - 1)]);
...

  {
    ...
    side: 'client',
    useQuiche: true, // Route to QuicheClient for a real QUIC handshake
    actions: (opts) => {
      return []; // Handled by QuicheClient
    },
    expected: 'PASSED',
  },
  ...
  {
    name: 'quic-multi-stream-virus-upload-download',
    category: 'QM',
    description: 'HTTP/3 upload + download all 22 virus files across concurrent streams (44 streams)',
    side: 'client',
    useQuiche: true,
    clientHandler: async (shim, host, logger, pcap, session) => {
      const { VIRUS_PAYLOADS } = require('./firewall-scenarios');
      const TOTAL = VIRUS_PAYLOADS.length * 2;
      let succeeded = 0;
      let dataExchanged = 0;
      const promises = [];
      ...
      logger.info(`[quic] Virus multi-stream: ${succeeded}/${TOTAL} streams OK, ${dataExchanged} bytes exchanged`);
  ...
  {
    name: 'quic-multi-stream-sb-upload-download',
    category: 'QN',
    description: 'HTTP/3 upload + download all sandbox payloads across concurrent streams (100 streams max)',
    side: 'client',
    useQuiche: true,
    clientHandler: async (shim, host, logger, pcap, session) => {
      ...
      logger.info(`[quic] Sandbox multi-stream: ${succeeded}/${TOTAL} streams OK, ${dataExchanged} bytes exchanged`);
...
function adaptActionForQUIC(action, pnCounter) {
  switch (action.type) {
    case 'send': {
      ...
      // Auto-fragment oversized CRYPTO payloads (e.g. PQC ClientHello) across multiple Initial packets
      if (data.length > MAX_CRYPTO_PER_INITIAL) {
        const quicActions = [];
        let offset = 0;
        let fragIndex = 0;
        const totalFrags = Math.ceil(data.length / MAX_CRYPTO_PER_INITIAL);
        const dcid = crypto.randomBytes(8);
        while (offset < data.length) {
          const chunk = data.slice(offset, offset + MAX_CRYPTO_PER_INITIAL);
          const act = {
            type: 'send',
            ...
            label: `[QUIC] CRYPTO fragment ${fragIndex + 1}/${totalFrags} offset=${offset} (${chunk.length}B)`,
            _dcid: dcid,
          };
          // First fragment carries full payload + DCID for Retry handling
          if (fragIndex === 0) {
            act._cryptoPayload = data;
          }
          quicActions.push(act);
          offset += MAX_CRYPTO_PER_INITIAL;
          fragIndex++;
        }
        return quicActions;
      }
      ...
      return {
        type: 'send',
        data: buildQuicInitialWithCrypto(action.data, { dcid, packetNumber: pnCounter.next++ }),
        label: action.label ? `[QUIC] ${action.label}` : '[QUIC] TLS data in Initial',
        _cryptoPayload: action.data, // Original TLS data for Retry retransmission
        _dcid: dcid,
      };
    }
...
function generateAdaptedQuicScenarios() {
  ...
  // -- Helpers to identify scenarios that should use the quiche engine
  //    (well-behaved QUIC client) instead of raw QUIC Initial wrapping.

  // Groups natively supported by the quiche library. PQC groups (MLKEM,
  // FrodoKEM, McEliece, hybrids) are NOT supported — those must use the
  // raw QUIC Initial path which crafts proper ClientHellos with PQC key shares.
  const QUICHE_SUPPORTED_GROUPS = new Set([
    'SECP256R1', 'SECP384R1', 'SECP521R1', 'X25519',
  ]);

  // Check if a TLS 1.3 scan scenario uses a group supported by quiche
  function isTls13ScanWithSupportedGroup(sc) {
    if (!sc.name.startsWith('scan-tls13-')) return false;
    // Extract group from description: "... + TLS_AES_128_GCM_SHA256 + SECP521R1"
    const m = sc.description.match(/\+\s+(\w+)$/);
    if (!m) return false;
    return QUICHE_SUPPORTED_GROUPS.has(m[1]);
  }

  function isSniScenario(sc) {
    const baseName = sc.name.replace(/-(small|pqc)-ch$/, '');
    return sc.category === 'K' || baseName.startsWith('ext-sni-');
  }

  // Track emitted quiche scenario names to deduplicate CH size variants
  const emittedQuicheNames = new Set();
  ...
  for (const sc of allScenarios) {
    ...
    // Skip PQC-CH / small-CH variants for clientHandler scenarios — quiche builds its
    // own ClientHello internally so CH size variants have no effect over QUIC.
    if (typeof sc.clientHandler === 'function' && /-(?:pqc|small)-ch$/.test(sc.name)) continue;

    // -- Skip PAN-TLS adapted scenarios — native pan-quic-* already covers these
    //    with proper quiche handshake. The adapted versions just timeout.
    // -- Skip APP scenarios — they use hardcoded TCP/TLS connect logic inside clientHandler.
    if (sc.category === 'PAN' || sc.category === 'APP') continue;

    // -- Route TLS 1.3 scans, SNI, and PAN-PQC scenarios through quiche for a full
    //    handshake. Raw Initial wrapping produces identical meaningless timeouts.
    if (isTls13ScanWithSupportedGroup(sc) || isSniScenario(sc) || sc.category === 'PAN-PQC') {
      // Skip -pqc-ch variants — quiche builds its own ClientHello
      if (/-pqc-ch$/.test(sc.name)) continue;

      // Deduplicate: strip -small-ch suffix to produce one scenario per base name
      const baseName = sc.name.replace(/-small-ch$/, '');
      const quicName = 'quic-tls-' + baseName;
      if (emittedQuicheNames.has(quicName)) continue;
      emittedQuicheNames.add(quicName);

      const quicCategory = TLS_TO_QUIC_CATEGORY[sc.category] || 'QG';
      const quicScenario = {
        name: quicName,
        category: quicCategory,
        description: sc.description.replace(/ \[small CH\]$/, '') + ' [via QUIC/quiche]',
        side: 'client',
        protocol: 'quic',
        useQuiche: true,
        expected: 'PASSED',
        expectedReason: 'Well-behaved QUIC client — handshake should complete',
      };
    ...
    let expected = sc.expected || 'DROPPED';
    // QUIC requires TLS 1.3 and rejects old ciphers/versions natively.
    if (sc.name.includes('scan-ssl') || sc.name.includes('scan-tls10') || sc.name.includes('scan-tls11') || sc.name.includes('scan-tls12')) {
      expected = 'DROPPED';
    }
