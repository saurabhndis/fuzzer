...
const { buildQuicInitialWithCrypto, buildQuicConnectionClose, encodeVarInt, protectPacket } = require('./quic-packet');
...
const { FW_SRV_SCENARIOS, FW_QUIC_SCENARIOS } = require('./firewall-scenarios');
const { SB_QUIC_SCENARIOS } = require('./sandbox-scenarios');
...
const QUIC_SCENARIOS = [
  ...
  // ── Multi-stream virus/sandbox upload+download scenarios ──────────────────
  {
    ...
    clientHandler: async (shim, host, logger, pcap, session) => {
      ...
      logger.info(`[quic] Virus multi-stream: uploads ${uploadsTransferred}/${uploads.length} transferred, ${uploadsBlocked} blocked; downloads ${downloadsTransferred}/${downloads.length} transferred, ${downloadsBlocked} blocked; ${dataExchanged} bytes exchanged`);
      return {
        status: 'PASSED',
        response: `${summarizeThreatTransfers({ protocol: 'HTTP/3', family: 'virus', total: downloads.length, transferred: downloadsTransferred, blocked: downloadsBlocked, bytes: dataExchanged })}; uploads ${uploadsTransferred}/${uploads.length} transferred, ${uploadsBlocked} blocked by FW`,
      };
    },
    expected: 'PASSED',
  },
  {
    ...
    clientHandler: async (shim, host, logger, pcap, session) => {
      ...
      logger.info(`[quic] Sandbox multi-stream: uploads ${uploadsTransferred}/${uploads.length} transferred, ${uploadsBlocked} blocked; downloads ${downloadsTransferred}/${downloads.length} transferred, ${downloadsBlocked} blocked; ${dataExchanged} bytes exchanged`);
      return {
        status: 'PASSED',
        response: `${summarizeThreatTransfers({ protocol: 'HTTP/3', family: 'sandbox', total: downloads.length, transferred: downloadsTransferred, blocked: downloadsBlocked, bytes: dataExchanged })}; uploads ${uploadsTransferred}/${uploads.length} transferred, ${uploadsBlocked} blocked by FW`,
      };
    },
    expected: 'PASSED',
  },

  // Removed: quic-post-handshake-{garbage,slowloris,http-smuggling}.
  // Those scenarios required raw 1-RTT stream writes (binary noise / 1-byte
  // drip / HTTP/1.1 syntax), which the current quiche-client shim cannot
  // emit — it framing-translates every write into a proper H3 DATA frame.
  // Without a raw-QUIC mode in the shim the scenarios degenerate into a
  // handshake-only check, which `quic-well-behaved-single-get` already covers.
];

// Push PAN QUIC scenarios
QUIC_SCENARIOS.push(...getPanSniScenarios('quic'));

// Push QUIC firewall and sandbox detection scenarios
QUIC_SCENARIOS.push(...FW_QUIC_SCENARIOS);
QUIC_SCENARIOS.push(...SB_QUIC_SCENARIOS);
