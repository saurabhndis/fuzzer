...
const QUIC_SCENARIOS = [
    ...
    clientHandler: async (shim, host, logger, pcap, session) => {
      ...
      const requests = Array.from({ length: STREAM_COUNT }, (_, i) => {
        ...
        return new Promise((resolve) => {
          ...
          req.on('error', (e) => { errors++; logger.warn(`Stream ${i} error: ${e.message}`); resolve(0); });
        });
      });

      await Promise.all(requests);
      if (errors > 0) return { status: 'ERROR', response: `${errors}/${STREAM_COUNT} streams failed` };
      const allCorrect = (await Promise.all(requests.map(p => p))).every(b => b === PAYLOAD_SIZE);
      return {
        status: 'PASSED',
        response: `${completed}/${STREAM_COUNT} GET streams OK, ${totalBytes} bytes total (${totalBytes / 1024}KB)`,
      };
    },
    expected: 'PASSED',
  },

  {
    ...
    clientHandler: async (shim, host, logger, pcap, session) => {
      ...
      const requests = payloads.map((payload, i) => {
        ...
        return new Promise((resolve) => {
          ...
          req.on('end', () => {
            ...
            else logger.warn(`Stream ${i}: echo mismatch (sent ${payload.length}B, got ${buf.length}B)`);
            resolve(buf.length);
          });
          req.on('error', (e) => { errors++; logger.warn(`Stream ${i} error: ${e.message}`); resolve(0); });
        });
      ...
      await Promise.all(requests);
      if (errors > 0) return { status: 'ERROR', response: `${errors}/${STREAM_COUNT} streams failed` };
      if (verified < STREAM_COUNT) return { status: 'ERROR', response: `Echo mismatch: only ${verified}/${STREAM_COUNT} streams verified` };
...

  {
    ...
    category: 'QZ',
    description: 'HTTP/3 DELETE: 100 concurrent streams each sending and echo-verifying 100KB body (max bidi stream limit)',
    side: 'client',
    useQuiche: true,
    clientHandler: async (shim, host, logger, pcap, session) => {
      const STREAM_COUNT = 100;
      ...
      if (errors > 0) return { status: 'ERROR', response: `${errors}/${STREAM_COUNT} streams failed` };
      if (verified < STREAM_COUNT) return { status: 'ERROR', response: `Echo mismatch: only ${verified}/${STREAM_COUNT} streams verified` };
      const totalBytes = STREAM_COUNT * PAYLOAD_SIZE * 2; // sent + echoed back
      return {
        status: 'PASSED',
        response: `${verified}/${STREAM_COUNT} DELETE streams echo-verified, ${totalBytes / 1024}KB exchanged`,
      };
    },
    expected: 'PASSED',
  },
