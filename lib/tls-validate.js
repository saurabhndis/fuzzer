...
/**
 * Detailed parser for ClientHello to extract parameters for scenario generation.
 */
function parseClientHello(body) {
  if (body.length < 38) return null;
  let off = 0;

  const version = (body[off] << 8) | body[off + 1]; off += 2;
  const random = body.subarray(off, off + 32); off += 32;
  const sidLen = body[off]; off++;
  const sessionId = body.subarray(off, off + sidLen); off += sidLen;

  const csLen = (body[off] << 8) | body[off + 1]; off += 2;
  const cipherSuites = [];
  for (let i = 0; i < csLen; i += 2) {
    if (off + i + 1 < body.length) {
      cipherSuites.push((body[off + i] << 8) | body[off + i + 1]);
    }
  }
  off += csLen;

  if (off >= body.length) return { version, random, sessionId, cipherSuites, compressionMethods: [], extensions: [] };
  const cmLen = body[off]; off++;
  const compressionMethods = Array.from(body.subarray(off, off + cmLen));
  off += cmLen;
  ...
  if (off + 2 <= body.length) {
    ...
    while (off + 4 <= extEnd) {
      const type = (body[off] << 8) | body[off + 1]; off += 2;
      const len = (body[off] << 8) | body[off + 1]; off += 2;
      const data = body.subarray(off, off + len);
  ...
  return {
    version, random, sessionId, cipherSuites, compressionMethods, extensions
  };
}

/**
 * Detailed parser for ServerHello to extract chosen parameters.
 */
function parseServerHello(body) {
  ...
  const sidLen = body[off]; off++;
  const sessionId = body.subarray(off, off + sidLen); off += sidLen;
  const cipherSuite = (body[off] << 8) | body[off + 1]; off += 2;
  const compressionMethod = body[off]; off++;
...
/**
 * Parse CertificateRequest handshake message.
 *
 * Body layout (TLS 1.2):
 *   certificate_types_len(1) | certificate_types(N)
 *   supported_signature_algorithms_len(2) | supported_signature_algorithms(M)
 *   certificate_authorities_len(2) | certificate_authorities(K)
 *
 * `certificate_authorities` is a list of DER-encoded DistinguishedName values
 * concatenated with no length prefix between entries (each DN is itself an
 * ASN.1 SEQUENCE so its length is self-describing).  We capture both the
 * raw concatenated bytes (`caListRaw`) and a parsed list of individual DN
 * buffers (`caList`) so the synthetic server can replay the captured CA
 * list verbatim — the DUT's cert-policy fingerprint includes the exact DN
 * bytes, so any padding/translation would be observable.
 */
function parseCertificateRequest(body) {
  try {
    let off = 0;
    const typesLen = body[off++];
    const types = Array.from(body.subarray(off, off + typesLen));
    off += typesLen;
    const sigAlgsLen = (body[off] << 8) | body[off + 1]; off += 2;
    const sigAlgs = [];
    for (let i = 0; i < sigAlgsLen; i += 2) {
      sigAlgs.push((body[off + i] << 8) | body[off + i + 1]);
    }
    off += sigAlgsLen;

    // certificate_authorities: optional in some captures; guard against
    // truncated CertReq messages.
    //
    // RFC 5246 §7.4.4 wire format:
    //   certificate_authorities_total_len (2 bytes)
    //     [ entry_len (2 bytes)  entry_bytes (entry_len bytes) ]*
    // Each entry's bytes are an opaque DistinguishedName (typically a DER
    // SEQUENCE starting with 0x30, but TLS treats it as opaque so we don't
    // require it). The previous walker assumed DNs were concatenated raw
    // (no per-entry length prefix), which is wrong for TLS 1.2 — captured
    // CA lists with valid entries showed as caCount=0.
    let caListRaw = Buffer.alloc(0);
    const caList = [];
    if (off + 2 <= body.length) {
      const caLen = (body[off] << 8) | body[off + 1]; off += 2;
      if (off + caLen <= body.length) {
        caListRaw = Buffer.from(body.subarray(off, off + caLen));
        let p = 0;
        while (p + 2 <= caListRaw.length) {
          const dnLen = (caListRaw[p] << 8) | caListRaw[p + 1];
          if (dnLen === 0 || p + 2 + dnLen > caListRaw.length) break;
          // Push the DN bytes only (without the 2-byte length prefix). The
          // builder side adds the length prefix back when writing the wire.
          caList.push(Buffer.from(caListRaw.subarray(p + 2, p + 2 + dnLen)));
          p += 2 + dnLen;
        }
      }
    }
    return { types, sigAlgs, caList, caListRaw };
  } catch (_) {
    return {};
  }
}
...
/**
 * Detect STARTTLS plain text before actual TLS records.
 * Returns { plain: Buffer, tls: Buffer } or null if no TLS record found.
 */
function parseSTARTTLS(buffer) {
  for (let i = 0; i < buffer.length - 5; i++) {
    // Look for TLS Record Header: [ContentType][VersionMajor:0x03][VersionMinor:0x00-0x04]
    if ((buffer[i] >= 0x14 && buffer[i] <= 0x18) && buffer[i + 1] === 0x03 && buffer[i + 2] <= 0x04) {
      return {
        plain: buffer.subarray(0, i),
        tls: buffer.subarray(i)
      };
    }
  }
  return null;
...
/**
 * Extract specific extension data from parsed extensions array.
 */
function getExtension(extensions, type) {
...
function parseSNI(data) {
  try {
    const listLen = (data[0] << 8) | data[1];
    let off = 2;
    while (off + 3 <= 2 + listLen && off + 3 <= data.length) {
      const nameType = data[off]; off++;
      const nameLen = (data[off] << 8) | data[off + 1]; off += 2;
      if (nameType === 0) return data.subarray(off, off + nameLen).toString('ascii');
      off += nameLen;
    }
  } catch (_) {}
  return null;
...
/**
 * Parse ALPN extension data to extract protocol names.
 */
function parseALPN(data) {
  try {
    ...
    let off = 2;
    const protocols = [];
    while (off < 2 + listLen && off < data.length) {
      const len = data[off]; off++;
      protocols.push(data.subarray(off, off + len).toString('ascii'));
      off += len;
    }
    return protocols;
  } catch (_) {}
...
/**
 * Parse a TLS Certificate handshake message body.
 * Format: [total_length:3]([cert_length:3][cert_der:N])*
 * Returns array of DER-encoded certificate Buffers.
 */
function parseCertificateChain(body) {
  if (!body || body.length < 3) return [];
  const totalLen = (body[0] << 16) | (body[1] << 8) | body[2];
  let off = 3;
  const certs = [];
  const end = Math.min(3 + totalLen, body.length);
  while (off + 3 <= end) {
    const certLen = (body[off] << 16) | (body[off + 1] << 8) | body[off + 2];
    off += 3;
    if (off + certLen > end) break;
    certs.push(Buffer.from(body.subarray(off, off + certLen)));
    off += certLen;
  }
  return certs;
}
...
module.exports = {
  validateClientHello,
  validateServerFlight,
  validateClientKeyExchange,
