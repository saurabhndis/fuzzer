...
class Logger {
  ...
  summary(results, report = null) {
    ...
    if (report) {
      const gradeColors = { A: COLORS.green, B: COLORS.blue, C: COLORS.yellow, D: COLORS.magenta, F: COLORS.red, I: COLORS.gray };
      ...
      const didNotRun = report.stats.error > 0
        ? `  ${COLORS.gray}DID NOT RUN: ${report.stats.error}${COLORS.reset}`
        : '';
      console.log(`  ${COLORS.green}PASS: ${report.stats.pass}${COLORS.reset}  ${COLORS.red}FAIL: ${report.stats.fail}${COLORS.reset}  ${COLORS.yellow}WARN: ${report.stats.warn}${COLORS.reset}  ${COLORS.gray}INFO: ${report.stats.info}${COLORS.reset}${didNotRun}`);
