...
//   INFO  — scenario ran, but no expected value was set to assert against
//   ERROR — scenario never ran (connection refused, harness fault, aborted).
//           Distinct from INFO: INFO is a result, ERROR is the absence of one,
//           and it is evidence neither for nor against the target.
//
// Overall grade:
...
//   I — Inconclusive: too little of the run actually executed to make any
//       claim about the target. A grade is a statement about the target's
//       security posture, and that statement is only meaningful when tests
//       produced verdicts — a run where nothing connected must not read as
//       "A — all tests passed".
...
function gradeResult(result, scenarioMeta) {
  ...
  // Aborted / Error — the scenario never produced a verdict. Deliberately not
  // INFO: lumping "didn't run" in with "ran, nothing to assert" is what let a
  // run where every scenario failed to connect still score an A.
  if (status === 'ERROR' || status === 'ABORTED') {
    return { grade: 'ERROR', severity, reason: `Scenario ${status.toLowerCase()}` };
  }

  // TLS alert statuses are proper rejection signals, but not success for clean pass scenarios.
  if (status === 'tls-alert-server') {
    if (expected === 'PASSED') {
      return { grade: 'WARN', severity, reason: 'Server sent fatal alert for valid input' };
    }
    return { grade: 'PASS', severity, reason: 'Server correctly sent fatal alert (protocol rejection)' };
  }
  if (status === 'tls-alert-client') {
    if (expected === 'PASSED') {
      return { grade: 'WARN', severity, reason: 'Client sent fatal alert for valid input' };
    }
...
function computeOverallGrade(gradedResults) {
  const stats = { pass: 0, fail: 0, warn: 0, info: 0, error: 0 };
  ...
  // Scenarios that never ran can't support any claim about the target. When
  // too few produced a verdict the honest answer is "inconclusive" — neither
  // A (nothing proved the target is sound) nor F (nothing proved it is not).
  const total = gradedResults.length;
  const executed = total - stats.error;
  const inconclusive = executed === 0 || stats.error > total / 2;

  // Both branches are F; hostDown is checked first because it produces a
  // critical finding of its own, and "target crashed" says more than
  // "critical vulnerability". Both precede the inconclusive test on purpose:
  // a crash makes every later scenario error out, and that must not soften
  // the verdict into "inconclusive".
  if (gradedResults.some(r => r.hostDown)) {
    grade = 'F';
    label = 'Target crashed during testing';
  } else if (failsBySeverity.critical > 0) {
    grade = 'F';
    label = 'Critical vulnerabilities detected';
  } else if (inconclusive) {
    grade = 'I';
    label = total === 0
      ? 'Inconclusive — no scenarios ran'
      : `Inconclusive — ${stats.error} of ${total} scenarios did not run`;
  ...
  } else if (stats.warn > executed * 0.3) {
    // Rate is over what actually ran — errored scenarios would otherwise
    // dilute the warn ratio and mask a genuinely warn-heavy run.
    grade = 'B';
    label = 'Mostly secure, some strict rejections';
  } else {
    grade = 'A';
    // Don't claim "all tests passed" when some never ran, even if the ones
    // that did were clean.
    label = stats.error > 0
      ? `All executed tests passed — ${stats.error} of ${total} did not run`
      : 'All tests passed — robust TLS implementation';
  }
