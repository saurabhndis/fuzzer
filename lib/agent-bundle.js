...
const ROOT_FILES = ['client.js', 'server.js', 'cert-verify-server.js', 'package.json'];
...
async function buildAgentBundle() {
  ...
  for (const f of files) {
    ...
    if (f.relativePath === 'package.json') {
      // Strip all deps — the agent only uses Node built-ins (http, tls, crypto…).
      // @currentspace/http3 is QUIC-only (lazy-loaded in unified-client/server),
      // xml2js is only used in main.js (Electron), neither is needed on remote.
      // Removing them means npm install is a no-op and requires no internet.
      const pkg = JSON.parse(fs.readFileSync(f.absolutePath, 'utf8'));
      delete pkg.devDependencies;
      delete pkg.dependencies;
      delete pkg.optionalDependencies;
      delete pkg.peerDependencies;
      delete pkg.main; // Electron entry, not needed on remote
      if (pkg.scripts) {
