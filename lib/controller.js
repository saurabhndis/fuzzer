...
const RUN_SCENARIO_TIMEOUT_MS = 210000; // must exceed the client's 180s safety timeout
const DEBUG_SNAPSHOT_TIMEOUT_MS = 2500;
...
class Controller {
  constructor() {
    ...
    this._lastTimeoutSnapshots = [];
    this._helperPairIdsByRole = { client: new Set(), server: new Set() };
    this._hasExplicitHelperPairs = false;
  }
   ...
   */
  async configure(role, scenarioNames, config, pcapScenarios = undefined, options = {}) {
    ...
    if (options && Array.isArray(options.helperPairIds)) {
      payload.helperPairIds = options.helperPairIds.map(v => String(v));
    }
   ...
   */
  async configureAll(clientScenarios, serverScenarios, clientConfig, serverConfig, clientPcapScenarios, serverPcapScenarios, helperPairs = {}) {
    const promises = [];
    const configured = { client: false, server: false };
    this._helperPairIdsByRole = {
      client: new Set((helperPairs.client || []).map(v => String(v))),
      server: new Set((helperPairs.server || []).map(v => String(v))),
    };
    this._hasExplicitHelperPairs = Array.isArray(helperPairs.client) || Array.isArray(helperPairs.server);
    ...
    if (hasClientWork && this.agents.client) {
      promises.push(this.configure('client', clientScenarios, clientConfig, clientPcapScenarios, {
        helperPairIds: helperPairs.client || [],
      }).then(() => { configured.client = true; }));
    }
    ...
    if (hasServerWork && this.agents.server) {
      promises.push(this.configure('server', serverScenarios, serverConfig, serverPcapScenarios, {
        helperPairIds: helperPairs.server || [],
      }).then(() => { configured.server = true; }));
    }
   ...
   */
  async runAll() {
    throw new Error('Legacy bulk distributed execution is disabled; use stepped distributed mode');
  }
  ...
  async runStepped(totalPairs) {
    ...
    for (let i = 0; i < pairCount; i++) {
      ...
      if (hasServer && serverName) {
        ...
        serverPromise =
          this._request(sa.host, sa.port, 'POST', '/run-scenario', { pairId, scenarioName: serverName }, sa.token, RUN_SCENARIO_TIMEOUT_MS)
        ...
        const ready = await this._waitForReady('server', 5000);
        if (!ready) {
          const failure = {
            role: 'server',
            pairId,
            scenarioName: serverName,
            phase: 'readiness',
            error: 'Server did not report ready within 5000ms',
          };
          this._stepFailures.push(failure);
          this._emitEvent('controller', { type: 'error', message: `Pair ${pairId} readiness failed: ${failure.error}` });
          this._request(sa.host, sa.port, 'POST', '/abort-scenario', { scenario: serverName, pairId }, sa.token, 2000)
            .catch(() => {});
          const settled = await Promise.allSettled(promises);
          for (const r of settled) {
            if (r.status === 'rejected') {
              this._stepFailures.push({
                pairId,
                error: r.reason && r.reason.message ? r.reason.message : String(r.reason),
              });
            } else if (r.value && r.value.error) {
              this._stepFailures.push({ pairId, ...r.value });
            }
          }
          continue;
        }
      ...
      if (hasClient && clientName) {
        ...
        clientPromise =
          this._request(ca.host, ca.port, 'POST', '/run-scenario', { pairId, scenarioName: clientName }, ca.token, RUN_SCENARIO_TIMEOUT_MS)
            .catch(err => ({ error: err.message, role: 'client', pairId, scenarioName: clientName }));
        promises.push(clientPromise);
      }

      // If both sides were dispatched, let the first completed side release the
      // peer. This makes helper-counterpart pairs (well-behaved server/client)
      // finish promptly instead of waiting for the passive side's full timeout.
      // Explicit peer-done actions still work; this is the controller-level
      // backstop for scenarios that do not include them.
      if (serverPromise && clientPromise) {
        const first = await Promise.race([
          serverPromise.then(() => 'server').catch(() => 'server'),
          clientPromise.then(() => 'client').catch(() => 'client'),
        ]);
        if (!this._stopped) {
          const clientIsHelper = this._isHelperScenario(clientName, 'client', pairId);
          const serverIsHelper = this._isHelperScenario(serverName, 'server', pairId);
          if (first === 'server' && clientIsHelper && clientName) {
            const ca = this.agents.client;
            this._request(ca.host, ca.port, 'POST', '/abort-scenario', { scenario: clientName, pairId }, ca.token, 2000)
              .catch(() => {});
          } else if (first === 'client' && serverIsHelper && serverName) {
            const sa = this.agents.server;
            this._request(sa.host, sa.port, 'POST', '/abort-scenario', { scenario: serverName, pairId }, sa.token, 2000)
              .catch(() => {});
          }
  ...
  _handleBarrierArrived(role, event) {
    const scenarioKey = event.runId !== undefined && event.runId !== null &&
      event.pairId !== undefined && event.pairId !== null
      ? `${event.runId}\u0000${event.pairId}`
      : (event.scenario || '');
  ...
  _postReleaseBarrier(role, scenario, label, reason) {
    const agent = this.agents[role];
    if (!agent) return;
    let payload = { scenario, label, reason };
    if (scenario && scenario.includes('\u0000')) {
      const [runId, pairId] = scenario.split('\u0000');
      payload = { scenario: null, label, reason, runId: Number(runId), pairId };
    }
    this._request(agent.host, agent.port, 'POST', '/release-barrier', payload, agent.token, 2000)
      .catch(() => {});
  }
  ...
  _handlePeerDone(role, event) {
    ...
    if (!peer) return;
    this._request(peer.host, peer.port, 'POST', '/abort-scenario', {
  ...
  _isHelperScenario(name, role, pairId) {
    if (this._hasExplicitHelperPairs) {
      const ids = role && this._helperPairIdsByRole ? this._helperPairIdsByRole[role] : null;
      return !!(ids && pairId !== undefined && pairId !== null && ids.has(String(pairId)));
    }
    return typeof name === 'string' && (
      name.startsWith('well-behaved-') ||
      name.startsWith('srv-quic-well-behaved') ||
      name.startsWith('fv-tls-well-behaved') ||
      name.includes('well-behaved-server') ||
      name.includes('well-behaved-client')
    );
  }

  get timeoutSnapshots() {
    return (this._lastTimeoutSnapshots || []).slice();
  }

  async _captureTimeoutSnapshot(host, port, token, requestMeta) {
    const capturedAt = new Date().toISOString();
    try {
      const snapshot = await this._request(host, port, 'GET', '/debug-state', null, token, DEBUG_SNAPSHOT_TIMEOUT_MS, { skipTimeoutSnapshot: true });
      const record = { capturedAt, host, port, request: requestMeta, snapshot };
      this._lastTimeoutSnapshots.push(record);
      if (this._lastTimeoutSnapshots.length > 20) this._lastTimeoutSnapshots.shift();
      this._emitEvent('controller', { type: 'timeout-debug', host, port, request: requestMeta, snapshot });
      return record;
    } catch (err) {
      const record = { capturedAt, host, port, request: requestMeta, error: err.message };
      this._lastTimeoutSnapshots.push(record);
      if (this._lastTimeoutSnapshots.length > 20) this._lastTimeoutSnapshots.shift();
      this._emitEvent('controller', { type: 'timeout-debug', host, port, request: requestMeta, error: err.message });
      return record;
    }
  }
   ...
   */
  _request(host, port, method, path, body, token, timeout = 10000, options = {}) {
    return new Promise((resolve, reject) => {
      ...
      req.on('timeout', async () => {
        ...
        settled = true;
        req.destroy();
        let message = `Request to ${host}:${port}${path} timed out`;
        if (!options.skipTimeoutSnapshot && path !== '/debug-state') {
          const record = await this._captureTimeoutSnapshot(host, port, token, { method, path, timeout, body });
          const agent = record && record.snapshot && record.snapshot.agent;
          const fuzzer = record && record.snapshot && record.snapshot.fuzzer;
          const currentAction = (agent && agent.currentAction) || (fuzzer && fuzzer.currentAction);
          if (agent && (agent.currentScenarioName || agent.currentPairId !== null)) {
            const actionSummary = currentAction
              ? `${currentAction.type || 'unknown'}:${currentAction.phase || 'unknown'}#${currentAction.index !== undefined ? currentAction.index : '?'}`
              : 'unknown-action';
            message += ` [debug scenario=${agent.currentScenarioName || 'n/a'} pair=${agent.currentPairId || 'n/a'} action=${actionSummary}]`;
          }
        }
