...
try {
  ...
  for (const [opensslName, groupId] of Object.entries(OPENSSL_PQC_GROUP_MAP)) {
    if (out.includes(opensslName)) WELL_BEHAVED_GROUPS.add(groupId);
  }
} catch {
  // OpenSSL not on PATH or call failed — leave WELL_BEHAVED_GROUPS at classical curves only
}
