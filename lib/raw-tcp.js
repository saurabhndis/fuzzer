...
function buildTCPHeader(srcPort, dstPort, seq, ack, flags, window, urgentPointer, payload) {
  const headerLen = 20;
...
function buildTCPPacket(srcIP, dstIP, srcPort, dstPort, seq, ack, flags, window, urgentPointer, payload, ipId) {
  const data = payload || Buffer.alloc(0);
  const tcpHeader = buildTCPHeader(srcPort, dstPort, seq, ack, flags, window, urgentPointer, data);
...
class RawTCPSocket extends EventEmitter {
  ...
  connect(window, timeout = 5000) {
    ...
    return new Promise((resolve, reject) => {
      ...
      this._sendPacket(TCPFlags.SYN, null).catch(reject);
    });
  }
