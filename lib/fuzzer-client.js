...
class FuzzerClient {
  constructor(opts = {}) {
    ...
    this.mergePcap = true; // always merge into one file
    this.pcap = null;
    ...
    if (this.pcapFileBase) {
      ...
      try {
        ...
        this.pcap = new PcapWriter(this.pcapFileBase, {
          role: 'client', append: false, clientIP, serverIP, serverPort: this.port,
        });
      } catch (e) {
  ...
  async runScenario(scenario) {
    ...
    // Pass the already-open pcap writer so no second file is created.
    if (scenario.useNodeTLS || scenario.useNodeH2 || scenario.useCustomClient) {
      const { UnifiedClient } = require('./unified-client');
      const uc = new UnifiedClient({
        host: this.host, port: this.port, timeout: this.timeout, delay: this.delay,
        logger: this.logger, pcapWriter: this.pcap, keylogFd: this._keylogFd,
      });
      return uc.runScenario(scenario);
    }

    const actions = typeof scenario.actions === 'function' ? scenario.actions({ hostname: this.host, timeout: this.timeout }) : scenario.actions;
    if (!actions) {
    ...
    try {
      ...
      for (let i = 0; i < actions.length; i++) {
        ...
        // `breakOnClose`. Once the socket is gone, replaying captured alerts
        // or app-data is dead code — short-circuit instead of burning recv
        // timeouts on a half-closed connection.
        if (action.breakOnClose && (connectionClosed || socket.destroyed)) {
          this.logger.info(`[fuzzer-client] Skipping remaining ${actions.length - i} action(s): connection closed`);
          break;
        }
        this.logger.info(`[fuzzer-client] Action ${i+1}/${actions.length}: ${action.type} (${action.label || ''})`);
      ...
      // Rejection detection:
      // 1. Post-fuzz encrypted alert or only application data (TLS 1.3 rejection)
      if (status === 'PASSED' && hasFuzzAction && postFuzzRecvData) {
      ...
      // Premature termination: if status is still PASSED but the connection was 
      // closed by the server without sending any response data, that's a rejection.
      if (status === 'PASSED' && connectionClosed && !rawResponse && !postFuzzRecvData) {
        status = 'DROPPED';
  ...
  _waitForData(socket, timeout, isClosedFn) {
    // this.logger.info(`[_waitForData] timeout=${timeout}`);
    return new Promise((resolve) => {
  ...
  close() {
    ...
    if (this._keylogFd !== null) { try { fs.closeSync(this._keylogFd); } catch (_) {} this._keylogFd = null; }
  }
}
