        ...
        switch (action.type) {
          ...
          case 'recv': {
            ...
            if (data && data.length > 0) {
              ...
              // Determine if this response represents a rejection (drop)
              const { parseRecords } = require('./record');
              const { records } = parseRecords(data);
              
              const hasAlert = records.some(r => r.type === 21); // ALERT
              const hasAppData = records.some(r => r.type === 23); // APPLICATION_DATA (likely Encrypted Alert in TLS 1.3)
              
              // If the server sent an alert or encrypted response AND then closed the connection,
              // we treat it as a DROPPED status (the server rejected the input).
              // This prevents "Server accepted malicious input" (FAIL) findings when the server 
              // actually sent a proper alert.
              if ((hasAlert || hasAppData) && connectionClosed) {
                status = 'DROPPED';
              }
