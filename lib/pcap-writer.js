...
/**
 * Detect the local outbound IP that would be used to reach a given target.
 * Scans OS network interfaces for a non-loopback address matching the target's
 * IP family, preferring addresses on the same subnet.
 */
function detectLocalIP(targetIP) {
  const os = require('os');
  const isV6 = targetIP.includes(':');
  const family = isV6 ? 'IPv6' : 'IPv4';
  const ifaces = os.networkInterfaces();

  // Parse target IP into octets for subnet matching (IPv4 only)
  const targetOctets = !isV6 ? targetIP.split('.').map(Number) : null;
  ...
  for (const addrs of Object.values(ifaces)) {
    for (const addr of addrs) {
      if (addr.family !== family || addr.internal) continue;

      if (!isV6 && targetOctets) {
        // Score by matching octets (simple prefix match)
        const octets = addr.address.split('.').map(Number);
        let score = 0;
        for (let i = 0; i < 4; i++) {
          if (octets[i] === targetOctets[i]) score++;
          else break;
        }
        if (score > bestScore) { bestScore = score; bestMatch = addr.address; }
      } else {
        // For IPv6, take the first non-internal address
        if (!bestMatch) bestMatch = addr.address;
      }
    }
  }

  return bestMatch || (isV6 ? '::1' : '127.0.0.1');
}
 ...
 * For remote targets, auto-detects the local outbound IP.
 */
function resolveIPs(host) {
  ...
  let clientIP;
  if (isLoopback) {
    clientIP = isIPv6 ? '::1' : '127.0.0.1';
  } else {
    // Auto-detect the local outbound IP for remote targets
    try {
      clientIP = detectLocalIP(serverIP);
    } catch (_) {
      clientIP = isIPv6 ? '::1' : '10.0.0.1';
    }
  }
...
class PcapWriter {
  ...
  writePacket(payload, direction, flags = TCPFlags.PSH | TCPFlags.ACK) {
    const data = this._toBuffer(payload);
    ...
    // Segment oversized application data across multiple TCP packets (with
    // sequence numbers that advance per chunk via _buildTCPHeader) instead of
    // truncating it. A single relay-proxy 'data' chunk can carry several 16 KB
    // TLS records (e.g. 64 KB payloads → ~65 KB ciphertext); truncating here
    // corrupted the record stream and broke keylog-based decryption in Wireshark.
    let offset = 0;
    do {
      const chunk = data.length > maxPayload ? data.slice(offset, offset + maxPayload) : data;
      const eth = this._buildEthernetHeader(dir);
      const ip = this._buildNetworkHeader(dir, TCP_HEADER_SIZE, chunk.length, 6);
      const tcp = this._buildTCPHeader(dir, flags, chunk.length);

      const checksum = this._computeTransportChecksum(dir, tcp, chunk, 6);
      tcp.writeUInt16BE(checksum, 16);

      const packet = Buffer.concat([eth, ip, tcp, chunk]);
