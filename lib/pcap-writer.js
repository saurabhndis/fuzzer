...
class PcapWriter {
  ...
  _writeGlobalHeader() {
    ...
    buf.writeUInt32BE(PCAP_MAGIC, 0);
    buf.writeUInt16BE(PCAP_VERSION_MAJOR, 4);
    buf.writeUInt16BE(PCAP_VERSION_MINOR, 6);
    buf.writeInt32BE(0, 8);  // thiszone
    buf.writeUInt32BE(0, 12); // sigfigs
    buf.writeUInt32BE(PCAP_SNAPLEN, 16);
    buf.writeUInt32BE(PCAP_LINKTYPE_ETHERNET, 20);
  ...
  writePacket(payload, direction, flags = TCPFlags.PSH | TCPFlags.ACK) {
    ...
    packetHeader.writeUInt32BE(tsSec, 0);
    packetHeader.writeUInt32BE(tsUsec, 4);
    packetHeader.writeUInt32BE(packet.length, 8);  // incl_len
    ...
    packetHeader.writeUInt32BE(Math.min(totalOrigLen, 0xFFFFFF), 12); // orig_len
  ...
  writeRawPacket(ipPacket, direction) {
    ...
    packetHeader.writeUInt32BE(tsSec, 0);
    packetHeader.writeUInt32BE(tsUsec, 4);
    packetHeader.writeUInt32BE(fullPacket.length, 8);  // incl_len
