...
const { SHUTDOWN_HOSTNAME } = require('./shutdown-signal');

function isShutdownPacket(msg) {
  if (!msg || msg.length < 10) return false;
  // Simplistic check for the shutdown SNI string in the UDP packet.
  // This works because fuzzer-generated shutdown packets aren't encrypted.
  return msg.toString('utf8').includes(SHUTDOWN_HOSTNAME);
}
...
class QuicFuzzerServer {
  ...
  async start() {
    ...
    await new Promise((resolve, reject) => {
      const bindWithRetry = (sock, retriesLeft) => {
        ...
        const bindAddr = this.bindAddress || ((this.hostname === 'localhost' || this.hostname === '::1') ? '::' : this.hostname);
  ...
  cancelScenarioWaits(reason = 'Scenario aborted') {
    if (!this._scenarioWaitCancelers || this._scenarioWaitCancelers.size === 0) return;
  ...
  _runHandlerScenario(scenario) {
    return new Promise((resolve) => {
      ...
      const scenarioTimeout = setTimeout(() => {
        if (!this.socket) return resolveResult(this._buildResult(scenario, 'TIMEOUT', 'Socket closed'));
        this.socket.removeListener('message', onMessage);
        this.logger.error(`Scenario "${scenario.name}" timed out — no client packet received.`);
        resolveResult(this._buildResult(scenario, 'TIMEOUT', 'No client packet within 60s'));
      }, 60000);
      const unregisterCancel = this._registerScenarioWaitCanceler((reason) => {
        clearTimeout(scenarioTimeout);
        if (this.socket) this.socket.removeListener('message', onMessage);
        resolveResult(this._buildResult(scenario, this.aborted ? 'ABORTED' : 'PEER_DONE', reason || 'Scenario aborted'));
      });

      const onMessage = (msg, rinfo) => {
        if (isShutdownPacket(msg)) {
          this.logger.info(`Shutdown signal received in QUIC packet (SNI: ${SHUTDOWN_HOSTNAME})`);
          clearTimeout(scenarioTimeout);
          this.socket.removeListener('message', onMessage);
          this.abort();
          resolveResult(this._buildResult(scenario, 'ABORTED', 'Server shutdown by client signal'));
          return;
        }
        ...
        try {
          ...
          const handlerDone = (value) => {
            const status = value && typeof value === 'object' && value.status ? value.status : 'PASSED';
            const response = value && typeof value === 'object' && value.response
              ? value.response
              : (typeof value === 'string' ? value : `Handler executed (client: ${rinfo.address}:${rinfo.port})`);
            this.logger.result(
              scenario.name, status, response, this._computeVerdict(status, scenario.expected || computeExpected(scenario).expected, response),
              scenario.expectedReason || '', false, 'pass', null
            );
            resolveResult(this._buildResult(scenario, status, response));
          };
          if (handlerResult && typeof handlerResult.then === 'function') {
            handlerResult.then(handlerDone).catch((e) => {
              this.logger.error(`Scenario handler error: ${e.message}`);
              resolveResult(this._buildResult(scenario, 'ERROR', e.message));
            });
          } else {
            handlerDone();
          }
  ...
  _runActionsScenario(scenario) {
    return new Promise((resolve) => {
      ...
      const scenarioTimeout = setTimeout(() => {
        ...
        this.logger.error(`Scenario "${scenario.name}" timed out — no client packet received.`);
        resolveResult(this._buildResult(scenario, 'TIMEOUT', 'No client packet within 30s'));
      }, 30000);
      const unregisterCancel = this._registerScenarioWaitCanceler((reason) => {
        clearTimeout(scenarioTimeout);
        if (this.socket) this.socket.removeListener('message', onFirstMessage);
        resolveResult(this._buildResult(scenario, this.aborted ? 'ABORTED' : 'PEER_DONE', reason || 'Scenario aborted'));
      });
      ...
      const onFirstMessage = async (msg, rinfo) => {
          ...
          switch (action.type) {
            ...
            case 'recv': {
              ...
              if (data && data.canceled) {
                status = this.aborted ? 'ABORTED' : 'PEER_DONE';
                lastResponse = data.reason || 'Scenario aborted';
              } else if (data) {
        ...
        resolveResult(this._buildResult(scenario, status, lastResponse || status));
      };
