...
function buildCertificateRequest(opts = {}) {
  const version = opts.recordVersion || Version.TLS_1_2;
  const types = opts.types || [0x01, 0x02, 0x40];
  const sigAlgs = opts.sigAlgs || [0x0403, 0x0804, 0x0401, 0x0201];
  // CA list: PCAP-replay path supplies raw bytes via `caListRaw` so the
  // exact DN encodings from the captured handshake are preserved.
  // Falls back to empty list (TLS 1.2 allows zero authorities) if not set.
  const caListRaw = Buffer.isBuffer(opts.caListRaw) ? opts.caListRaw : Buffer.alloc(0);

  const parts = [];
  parts.push(Buffer.from([types.length]));
  parts.push(Buffer.from(types));

  const saBuf = Buffer.alloc(2 + sigAlgs.length * 2);
  saBuf.writeUInt16BE(sigAlgs.length * 2, 0);
  for (let i = 0; i < sigAlgs.length; i++) {
    saBuf.writeUInt16BE(sigAlgs[i], 2 + i * 2);
  }
  parts.push(saBuf);

  // certificate_authorities: 2-byte length prefix + concatenated DER DNs
  const caLen = Buffer.alloc(2);
  caLen.writeUInt16BE(caListRaw.length, 0);
  parts.push(caLen);
  if (caListRaw.length > 0) parts.push(caListRaw);

  const body = Buffer.concat(parts);
  return buildHandshakeRecord(HandshakeType.CERTIFICATE_REQUEST, body, version);
}
...
function buildServerHelloBody(opts = {}) {
  const version = opts.version || Version.TLS_1_2;
  const random = opts.random || crypto.randomBytes(32);
  const sessionId = opts.sessionId !== undefined ? opts.sessionId : crypto.randomBytes(32);
  const cipherSuite = opts.cipherSuite || CipherSuite.TLS_ECDHE_RSA_WITH_AES_128_GCM_SHA256;
  const compressionMethod = opts.compressionMethod !== undefined ? opts.compressionMethod : CompressionMethod.NULL;
  ...
  // Extensions (minimal)
  const extensions = [];
  const extraExtensions = Array.isArray(opts.extraExtensions) ? opts.extraExtensions : [];
  const hasRenegotiationInfo = extraExtensions.some(ext => ext && ext.type === ExtensionType.RENEGOTIATION_INFO);
  if (opts.includeDefaultRenegotiationInfo !== false && !hasRenegotiationInfo) {
    extensions.push(buildExtension(ExtensionType.RENEGOTIATION_INFO, Buffer.from([0x00])));
  }
...
function buildCertificate(opts = {}) {
  // If a real DER cert or chain is provided, use it verbatim.
  if (Array.isArray(opts.certs) && opts.certs.length > 0) {
    return buildCertificateMessage(opts.certs, opts);
  }
  if (opts.cert) {
