...
/**
 * Generate a valid ML-KEM encapsulation key using Node.js crypto.
 */
function generateMLKEMKey(mlkemParam) {
  ...
  return spki.slice(info.spkiHeaderSize); // Strip SPKI header to get raw ek
}
...
function generateKeyShareData(group, keySize) {
  ...
  if (standalone) {
    const { publicKey } = crypto.generateKeyPairSync(standalone.algo);
    const spki = publicKey.export({ type: 'spki', format: 'der' });
    return spki.slice(standalone.spkiHeaderSize);
  }
