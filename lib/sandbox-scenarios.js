...
function generateTLSSandboxScenarios() {
  return RESPONSE_PAYLOADS.map(payload => ({
    ...
    expected: 'PASSED',
    expectedReason: 'Pass if the sandbox payload downloads successfully; also pass if the firewall blocks, resets, or replaces it after the TLS handshake.',
  }));
}
...
function generateH2SandboxScenarios() {
  return RESPONSE_PAYLOADS.map(payload => ({
    ...
    expected: 'PASSED',
    expectedReason: 'Pass if the sandbox payload downloads successfully; also pass if the firewall blocks, resets, or replaces it after the HTTP/2 handshake.',
  }));
}
...
function generateQuicSandboxScenarios() {
  return RESPONSE_PAYLOADS.map(payload => ({
    ...
    expected: 'PASSED',
    expectedReason: 'Pass if the sandbox payload downloads successfully; also pass if the firewall blocks, resets, or replaces it after the HTTP/3 handshake.',
  }));
}
