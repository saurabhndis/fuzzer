...
function generateTLSSandboxScenarios() {
  return RESPONSE_PAYLOADS.map(payload => ({
    ...
    clientHandler: async (socket, host, logger) => {
      ...
      logger.info(`[sb] ${payload.id}: sent ${data.length}B (${payload.contentType}), received ${resp.buf.length}B (${resp.outcome}${resp.errMsg ? `: ${resp.errMsg}` : ''})`);
      ...
      if (resp.buf.length === 0) {
        ...
        return { status: 'TIMEOUT', response: `No response within 8s — inconclusive (could be firewall, slow peer, or transport issue)` };
      }
      ...
      return { status: 'PASSED', response: `Not blocked — ${resp.buf.length}B response echoed (firewall did not sandbox ${payload.name})` };
    },
    expected: 'DROPPED',
...
function generateH2SandboxScenarios() {
  return RESPONSE_PAYLOADS.map(payload => ({
    ...
    clientHandler: async (session, host, logger) => {
      ...
      logger.info(`[h2-sb] ${payload.id}: sent ${data.length}B (${payload.contentType}), status=${result.status} resp=${result.data.length}B (${result.outcome}${result.error ? `: ${result.error}` : ''})`);
      ...
      if (result.status === 0) {
        ...
        return { status: 'TIMEOUT', response: `No HTTP/2 response within 8s — inconclusive (could be firewall, slow peer, or transport issue)` };
      }
...
function generateQuicSandboxScenarios() {
  return RESPONSE_PAYLOADS.map(payload => ({
    name: `quic-sb-${payload.id}`,
    category: 'QN',
    description: `QUIC Sandbox: ${payload.name} — ${payload.description}`,
    side: 'client',
    protocol: 'quic',
    useQuiche: true,
    clientHandler: async (shim, host, logger, pcap, session) => {
      ...
      logger.info(`[quic-sb] ${payload.id}: sent ${data.length}B (${payload.contentType}), status=${result.status} resp=${result.data.length}B (${result.outcome}${result.error ? `: ${result.error}` : ''})`);
      ...
      if (result.status === 0) {
        ...
        return { status: 'TIMEOUT', response: `No HTTP/3 response within 8s — inconclusive (could be firewall, slow peer, or transport issue)` };
      }
      if (result.status === 403 || result.status === 406 || result.status === 451) {
        return { status: 'DROPPED', response: `Blocked (HTTP ${result.status}) — firewall rejected response content` };
      }
    ...
    expected: 'DROPPED',
    expectedReason: `Firewall should sandbox/block server response over QUIC/HTTP3 containing: ${payload.description}`,
  }));
}

const SB_TLS_SCENARIOS = generateTLSSandboxScenarios();
const SB_H2_SCENARIOS = generateH2SandboxScenarios();
const SB_QUIC_SCENARIOS = generateQuicSandboxScenarios();
