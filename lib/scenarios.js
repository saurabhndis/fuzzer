...
const SCENARIOS = [
  ...
  },
  {
    ...
    side: 'client',
    expected: 'DROPPED',
    expectedReason: 'X25519Kyber768Draft (0x6399) is the pre-standard Chrome codepoint; OpenSSL 3.5+ ships only the final X25519MLKEM768 (0x11EC), so a well-behaved server should reject this with a handshake alert.',
    actions: (opts) => [
      { type: 'send', data: hs.buildClientHello({
        hostname: opts.hostname,
        extraExtensions: [
