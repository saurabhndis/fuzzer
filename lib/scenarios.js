...
const { PCAP_CATEGORY, PCAP_CATEGORY_LABEL, getPcapScenario, listPcapScenarios, getPcapScenariosByCategory } = require('./pcap-test-cases');
...
const CATEGORIES = {
  ...
  ...PAN_PQC_CATEGORIES,
  ...APP_CATEGORIES,
  [PCAP_CATEGORY]: PCAP_CATEGORY_LABEL,
};
...
// PCAP-CASE = user-saved test cases (opt-in, loaded from pcap-test-cases/ directory)
const CATEGORY_DEFAULT_DISABLED = new Set(['Z', 'SRV', PCAP_CATEGORY, 'RA', 'RB', 'RC', 'RD', 'RE', 'RF', 'RG']);
...
const SCENARIOS = [
  ...
  // ===== Category F: Record Layer Attacks =====
  {
    ...
    expected: 'DROPPED',
    expectedReason: 'The local strict helper stack rejects this record-version variant with a fatal alert, so a clean rejection is the deterministic expectation here.',
  },
  {
  ...
  // ===== Category J: Post-Quantum Cryptography (PQC) Fuzzing =====
  {
    ...
    actions: (opts) => [
      ...
      { type: 'recv', timeout: 5000, preserveAcceptSignal: true, statusOnAcceptSignal: 'PASSED', stopAfterAcceptSignal: true },
    ],
  },
  {
  ...
  },
  {
    ...
    side: 'client',
    expected: 'PASSED',
    expectedReason: 'Modern TLS 1.3 stacks can respond to the obsolete X25519Kyber768 draft codepoint with a graceful fallback, typically a HelloRetryRequest or ServerHello selecting a supported classical/final group instead of an immediate hard fail.',
    actions: (opts) => [
      { type: 'send', data: hs.buildClientHello({
        hostname: opts.hostname,
        extraExtensions: [
  ...
  },
  {
    ...
    side: 'client',
    expected: 'DROPPED',
    expectedReason: 'A malformed ML-KEM key_share length should be rejected with a fatal TLS alert.',
    actions: (opts) => [
      { type: 'send', data: hs.buildClientHello({
        hostname: opts.hostname,
        extraExtensions: [
  ...
  },
  {
    ...
    side: 'client',
    expected: 'DROPPED',
    expectedReason: 'An oversized PQC key_share should be rejected with a fatal TLS alert.',
    actions: (opts) => [
      { type: 'send', data: hs.buildClientHello({
        hostname: opts.hostname,
        extraExtensions: [
  ...
  },
  {
    ...
    side: 'client',
    expected: 'DROPPED',
    expectedReason: 'Unknown PQC named groups should be rejected with a fatal TLS alert.',
    actions: (opts) => [
      { type: 'send', data: hs.buildClientHello({
        hostname: opts.hostname,
        extraExtensions: [
  ...
  },
  {
    ...
    expected: 'TIMEOUT',
    expectedReason: 'Server waits for remaining handshake fragments, leading to timeout rather than immediate drop',
  },
  {
  ...
  },
  {
    ...
    useNodeTLS: true,
    clientHandler: async (socket, host, logger) => {
      const readResponse = (timeoutMs) => new Promise((resolve) => {
        let buf = Buffer.alloc(0);
        let settled = false;
        const done = () => {
          if (settled) return;
        ...
        const onData = (d) => {
          ...
          if (buf.length >= headerEnd + 4 + cl) done();
        };
        const onEnd = () => done();
        const onClose = () => done();
      ...
      for (let i = 0; i < totalRequests; i++) {
        ...
        const connectionHeader = i === totalRequests - 1 ? 'close' : 'keep-alive';
        socket.write(`GET /${i} HTTP/1.1\r\nHost: ${host}\r\nConnection: ${connectionHeader}\r\n\r\n`);
        const data = await readResponse(1500);
        if (data.length > 0 && data.toString().includes('200')) {
          successCount++;
          continue;
        }
        // Once the keep-alive chain breaks, stop early instead of spending
        // the rest of the scenario waiting on a dead connection.
        break;
      }
...
function generateCHSizeVariants() {
  ...
  for (const sc of clientScenarios) {
    ...
    // Skip PQC variant for slow-drip: sending ~10KB one byte/20ms takes ~200s, causing timeouts
    if (!sc.name.includes('slow-drip')) {
      variants.push({
        ...sc,
        name: sc.name + '-pqc-ch',
...
// Push Certificate Verification (CRL/OCSP) Scenarios
const { CV_SERVER_SCENARIOS, CV_CLIENT_SCENARIOS, CV_CATEGORIES, CV_CATEGORY_SEVERITY } = require('./cert-verify/scenarios');
SCENARIOS.push(...CV_SERVER_SCENARIOS);
SCENARIOS.push(...CV_CLIENT_SCENARIOS);
Object.assign(CATEGORIES, CV_CATEGORIES);
Object.assign(CATEGORY_SEVERITY, CV_CATEGORY_SEVERITY);

function getScenario(name) {
  return SCENARIOS.find(s => s.name === name) || getScanScenario(name) || getPcapScenario(name);
}
...
function getScenariosByCategory(cat) {
  ...
  if (catCode === PCAP_CATEGORY) {
    return getPcapScenariosByCategory(catCode, { includePending: true });
  }
...
function listScenarios() {
  ...
  // Add PCAP scenarios (include pending for listing)
  const { scenarios: pcapScenarios } = listPcapScenarios({ includePending: true });
  for (const [cat, items] of Object.entries(pcapScenarios)) {
    if (items.length > 0) {
      if (!grouped[cat]) grouped[cat] = [];
      grouped[cat] = grouped[cat].concat(items);
    }
