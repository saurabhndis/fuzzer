...
class WellBehavedServer {
  ...
  async startTLS() {
    ...
    this._server.on('connection', (rawSocket) => {
      ...
      const upgradeToTLS = (unshiftBuffer) => {
        if (isUpgrading) return;
        isUpgrading = true;
        rawSocket.removeAllListeners('data');
        if (unshiftBuffer && unshiftBuffer.length > 0) {
          rawSocket.unshift(unshiftBuffer);
        }
      ...
      let greetingTimer = setTimeout(() => {
        if (!isUpgrading && !rawSocket.destroyed && bufferedData.length === 0) {
          // Send a generic greeting that satisfies both SMTP and FTP StartTLS clients.
          rawSocket.write("220 Welcome to Test Server\r\n");
        }
      }, 50);
      ...
      rawSocket.on('data', (chunk) => {
        ...
        const cmd = bufferedData.toString('utf8').toUpperCase();

        // LDAP StartTLS (BER OID 1.3.6.1.4.1.1466.20037)
        const ldapStartTlsMagic = Buffer.from("312e332e362e312e342e312e313436362e3230303337", "hex");
        if (bufferedData.includes(ldapStartTlsMagic)) {
          // Send LDAP StartTLS Success Response
          rawSocket.write(Buffer.from("300c02010178070a010004000400", "hex"));
          // Discard bufferedData to prevent command injection (CVE-2011-0411 variants)
          upgradeToTLS(null);
          return;
        }
        ...
        if (cmd.includes('EHLO') || cmd.includes('HELO')) {
          rawSocket.write("250-localhost\r\n250 STARTTLS\r\n");
          if (cmd.includes('STARTTLS')) {
            rawSocket.write("220 Ready to start TLS\r\n");
            upgradeToTLS(null); // Discard remaining buffer (CVE-2011-0411 mitigation)
            return;
          }
          bufferedData = Buffer.alloc(0); // Clear buffer, wait for STARTTLS
          return;
        }
        if (cmd.includes('STARTTLS')) {
          rawSocket.write("220 Ready to start TLS\r\n");
          upgradeToTLS(null); // Discard remaining buffer
          return;
        }

        // FTP AUTH TLS
        if (cmd.includes('AUTH TLS')) {
          rawSocket.write("234 Ready\r\n");
          upgradeToTLS(null); // Discard remaining buffer
          return;
        }

        // Standard TLS Client Hello (0x16 0x03) or SSLv2 Client Hello (0x80)
        if (bufferedData.length >= 2) {
          const b0 = bufferedData[0];
          const b1 = bufferedData[1];
          if ((b0 === 0x16 && b1 === 0x03) || (b0 >= 0x80 && b1 >= 0x01)) {
            upgradeToTLS(bufferedData);
            return;
          }
        }
      });
