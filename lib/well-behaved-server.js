...
class WellBehavedServer {
  ...
  async startTLS() {
    // Node.js built-in TLS does not support PQC named groups (X25519MLKEM768,
    // MLKEM768, etc.). When PQC fuzzing scenarios run against the Node TLS
    // server they always get an alert because Node rejects unknown key shares.
    // If OpenSSL with PQC support is available on this system, prefer
    // spawning `openssl s_server` so PQC ClientHello messages are accepted.
    // Opt-out via WIRESTRIKE_NO_OPENSSL_TLS=1 if Node-only behavior is desired.
    if (!process.env.WIRESTRIKE_NO_OPENSSL_TLS) {
      const groups = getOpenSSLGroups();
      const hasPQC = groups && /MLKEM/i.test(groups);
      if (hasPQC) {
        try {
          await this._startOpenSSLTLS(groups);
          return;
        } catch (e) {
          if (this.logger) this.logger.info(`[local-server] OpenSSL s_server start failed (${e.message}), falling back to Node TLS`);
        }
      }
    }
  ...
  // Spawn `openssl s_server` so PQC key shares are accepted. Used by startTLS()
  // when OpenSSL with PQC (MLKEM*) support is detected on the host.
  async _startOpenSSLTLS(groups) {
    this._ensureCerts();
    ...
    if (this._maxVersion) {
      // Map e.g. 'TLSv1.2' → '-tls1_2'
      const m = /TLSv?1[._]?(\d)/i.exec(this._maxVersion);
      if (m) args.push(`-tls1_${m[1]}`);
    }
    if (this._ciphers) args.push('-cipher', this._ciphers);

    const proc = spawn('openssl', args, { stdio: ['ignore', 'pipe', 'pipe'] });
    this._opensslProc = proc;
    this._actualPort = this.port;

    // Wait for the server to be listening — openssl prints "ACCEPT" once ready,
    // but with -quiet it stays silent, so race a short port-probe instead.
    await new Promise((resolve, reject) => {
      ...
      proc.on('exit', (code) => {
        if (!settled) settle(() => reject(new Error(`openssl s_server exited prematurely (code=${code})`)));
      });
      ...
      setTimeout(probe, 50);
      setTimeout(() => settle(() => reject(new Error('openssl s_server did not become ready in 3s'))), 3000);
    });

    if (this.logger) this.logger.info(`[local-server] OpenSSL s_server listening on port ${this._actualPort} (PQC groups enabled)`);
  }
  ...
  stop() {
    ...
    if (this._opensslProc) {
      try { this._opensslProc.kill('SIGKILL'); } catch (_) {}
      this._opensslProc = null;
    }
    if (this._quicSocket) {
