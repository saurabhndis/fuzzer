...
function unprotectPacket(packet, keys) {
  try {
    ...
    require('fs').appendFileSync('debug.log', "unprotectPacket exception: " + e.stack + "\n");
    return null;
  }
}
  ...
  try {
    while (off < payload.length) {
      ...
      if (frameType === 0x02 || frameType === 0x03) { // ACK
        ...
        const { length: largestLen } = decodeVarInt(payload, off); off += largestLen;
        if (off >= payload.length) break;
        const { length: delayLen } = decodeVarInt(payload, off); off += delayLen;
        if (off >= payload.length) break;
        const { value: rangeCount, length: countLen } = decodeVarInt(payload, off); off += countLen;
        if (off >= payload.length) break;
        const { length: firstRangeLen } = decodeVarInt(payload, off); off += firstRangeLen;
        if (off >= payload.length) break;
        for (let i = 0; i < rangeCount; i++) {
          const { length: gapLen } = decodeVarInt(payload, off); off += gapLen;
          if (off >= payload.length) break;
          const { length: rLen } = decodeVarInt(payload, off); off += rLen;
          if (off >= payload.length) break;
        }
        if (frameType === 0x03) {
          const { length: ect0Len } = decodeVarInt(payload, off); off += ect0Len;
          if (off >= payload.length) break;
          const { length: ect1Len } = decodeVarInt(payload, off); off += ect1Len;
          if (off >= payload.length) break;
          const { length: ecnCeLen } = decodeVarInt(payload, off); off += ecnCeLen;
          if (off >= payload.length) break;
        }
        continue;
      }
      ...
      if (frameType === 0x1c || frameType === 0x1d) { // CONNECTION_CLOSE
        ...
        off += rlLen;
        if (off + reasonLen > payload.length) break;
    ...
    findings.push('MalformedPayload');
  }
...
// Maximum CRYPTO payload bytes that fit in a single 1200-byte QUIC Initial packet
// after accounting for Long Header (~23B), CRYPTO frame header (~5B), and AEAD tag (16B).
const MAX_CRYPTO_PER_INITIAL = 1100;

module.exports = PacketBuilder;
module.exports.MAX_CRYPTO_PER_INITIAL = MAX_CRYPTO_PER_INITIAL;
