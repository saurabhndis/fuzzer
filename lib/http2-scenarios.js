...
//   AH — Connectivity Probes          (info,     client)
...
const { summarizeThreatTransfers } = require('./threat-transfer-verdict');
...
const HTTP2_SCENARIOS = [
  ...
  {
    name: 'h2-push-promise-padded-truncated',
    category: 'AD',
    description: 'Padded PUSH_PROMISE Truncated (client→server) — PADDED flag set with frame length = pad_length + 1; '
      + 'no room for the 4-byte Promised Stream ID (RFC §6.6 FRAME_SIZE_ERROR)',
    side: 'client',
    actions: () => {
      const streamId = 1;
      const padLen = 10;
      // Payload: 1-byte pad length + 10 bytes padding = 11 bytes total
      // Missing: the 4-byte Promised Stream ID that must follow the pad length byte
      const rawPayload = Buffer.alloc(1 + padLen);
      rawPayload.writeUInt8(padLen, 0);
      // Bytes 1..10 are zero padding — no promised stream ID at all
      const frame = Buffer.concat([
        createFrameHeader(rawPayload.length, FrameType.PUSH_PROMISE, Flag.PADDED | Flag.END_HEADERS, streamId),
        rawPayload,
      ]);
      return [
        { type: 'send', data: prefaceBuffer(), label: 'H2 Connection Preface + SETTINGS' },
        { type: 'send', data: frame, label: '[FUZZ] PUSH_PROMISE PADDED with payload = pad_length+1 (0 bytes for Promised Stream ID)' },
        { type: 'recv', timeout: 5000 },
      ];
    },
    expected: 'DROPPED',
    expectedReason: 'Server must send GOAWAY FRAME_SIZE_ERROR — padded PUSH_PROMISE payload too short for 4-byte Promised Stream ID (RFC §6.6)',
  },

  {
    name: 'h2-push-promise-padded-short-id',
    category: 'AD',
    description: 'Padded PUSH_PROMISE Short ID (client→server) — PADDED flag set with frame length = pad_length + 3; '
      + 'only 2 of the required 4 bytes for Promised Stream ID (RFC §6.6 FRAME_SIZE_ERROR)',
    side: 'client',
    actions: () => {
      const streamId = 1;
      const padLen = 10;
      // Payload: 1-byte pad length + 2 bytes (partial stream ID) + 10 bytes padding = 13 bytes
      // Needed: 1 + 4 + 10 = 15 bytes — we're 2 bytes short
      const rawPayload = Buffer.alloc(1 + 2 + padLen);
      rawPayload.writeUInt8(padLen, 0);
      rawPayload.writeUInt16BE(0x0002, 1); // partial promised stream ID (only 2 of 4 bytes)
      // Bytes 3..12 are zero padding
      const frame = Buffer.concat([
        createFrameHeader(rawPayload.length, FrameType.PUSH_PROMISE, Flag.PADDED | Flag.END_HEADERS, streamId),
        rawPayload,
      ]);
      return [
        { type: 'send', data: prefaceBuffer(), label: 'H2 Connection Preface + SETTINGS' },
        { type: 'send', data: frame, label: '[FUZZ] PUSH_PROMISE PADDED with payload = pad_length+3 (only 2 of 4 bytes for Promised Stream ID)' },
        { type: 'recv', timeout: 5000 },
      ];
    },
    expected: 'DROPPED',
    expectedReason: 'Server must send GOAWAY FRAME_SIZE_ERROR — padded PUSH_PROMISE has only 2 of 4 required bytes for Promised Stream ID (RFC §6.6)',
  },
...

  {
    name: 'h2-ping-h2c',
    category: 'AH',
    description: 'H2C Connectivity Probe — cleartext HTTP/2 connection test to verify server is reachable',
    side: 'client',
    actions: () => [{ type: 'probe', label: 'H2C cleartext connection test' }],
    expected: 'CONNECTED',
    expectedReason: 'Cleartext HTTP/2 connection should succeed if server is reachable',
  },

  // ALPN and TLS version probe scenarios removed — not applicable to cleartext h2c
...

  {
    ...
    serverHandler: (stream, session, log) => {
      ...
      // Omit stream.respond() to prevent duplicate/cleartext frame leakage
    },
    expected: 'DROPPED',
  ...
  {
    name: 'h2-server-push-promise-padded-truncated',
    category: 'H2S',
    description: 'Padded PUSH_PROMISE Truncated — PADDED flag set with frame length = pad_length + 1; '
      + 'no room for the 4-byte Promised Stream ID (RFC §6.6 FRAME_SIZE_ERROR)',
    side: 'server',
    useRawTLS: true,
    serverHandler: (stream, session, log) => {
      log('Sending PUSH_PROMISE with PADDED flag but truncated payload (no promised stream ID)...');
      // Craft raw payload: 1-byte pad length + padding bytes only — no 4-byte promised stream ID
      const padLen = 10;
      const rawPayload = Buffer.alloc(1 + padLen); // total = 11 bytes = padLen + 1
      rawPayload.writeUInt8(padLen, 0);            // Pad Length field
      // Bytes 1..10 are padding (zeros) — no promised stream ID at all

      // Build frame: PUSH_PROMISE (0x5) with PADDED (0x08) | END_HEADERS (0x04)
      const frame = Buffer.concat([
        createFrameHeader(rawPayload.length, FrameType.PUSH_PROMISE, Flag.PADDED | Flag.END_HEADERS, stream.id),
        rawPayload,
      ]);
      writeRawFrame(session, frame);
      log('Truncated padded PUSH_PROMISE sent (0 bytes for stream ID) — client must GOAWAY FRAME_SIZE_ERROR.');
      // Omit stream.respond() to prevent duplicate/cleartext frame leakage
    },
    expected: 'DROPPED',
    expectedReason: 'Client must send GOAWAY with FRAME_SIZE_ERROR — padded PUSH_PROMISE payload too short for 4-byte Promised Stream ID (RFC §6.6)',
  },

  {
    name: 'h2-server-push-promise-padded-short-id',
    category: 'H2S',
    description: 'Padded PUSH_PROMISE Short ID — PADDED flag set with frame length = pad_length + 3; '
      + 'only 2 of the required 4 bytes for Promised Stream ID (RFC §6.6 FRAME_SIZE_ERROR)',
    side: 'server',
    useRawTLS: true,
    serverHandler: (stream, session, log) => {
      log('Sending PUSH_PROMISE with PADDED flag and partial promised stream ID (2 of 4 bytes)...');
      const padLen = 10;
      // Payload: 1-byte pad length + 2 bytes (partial stream ID) + 10 bytes padding = 13 bytes
      // Needed: 1 + 4 + 10 = 15 bytes — we're 2 bytes short
      const rawPayload = Buffer.alloc(1 + 2 + padLen);
      ...
      writeRawFrame(session, frame);
      log('Short-ID padded PUSH_PROMISE sent (2 of 4 bytes for stream ID) — client must GOAWAY FRAME_SIZE_ERROR.');
      // Omit stream.respond() to prevent duplicate/cleartext frame leakage
    },
    expected: 'DROPPED',
    expectedReason: 'Client must send GOAWAY with FRAME_SIZE_ERROR — padded PUSH_PROMISE has only 2 of 4 required bytes for Promised Stream ID (RFC §6.6)',
  },
      ...
      for (let i = 0; i < VIRUS_PAYLOADS.length; i++) {
        ...
        promises.push(new Promise((resolve) => {
          try {
            ...
            req.on('end', () => resolve({ kind: 'upload', transferred: status >= 200 && status < 400, blocked: status === 0 || status >= 400, bytes: body.length + respLen }));
            req.on('error', () => resolve({ kind: 'upload', transferred: false, blocked: true, bytes: 0 }));
            setTimeout(() => resolve({ kind: 'upload', transferred: status >= 200 && status < 400, blocked: status === 0 || status >= 400, bytes: body.length + respLen }), 10000);
            req.write(body);
            req.end();
          } catch { resolve({ kind: 'upload', transferred: false, blocked: true, bytes: 0 }); }
        }));
        ...
        promises.push(new Promise((resolve) => {
          try {
            ...
            req.on('response', (h) => { status = h[':status']; });
            req.on('data', (d) => { respLen += d.length; });
            req.on('end', () => resolve({ kind: 'download', transferred: status >= 200 && status < 400, blocked: status === 0 || status >= 400, bytes: respLen }));
            req.on('error', () => resolve({ kind: 'download', transferred: false, blocked: true, bytes: 0 }));
            setTimeout(() => resolve({ kind: 'download', transferred: status >= 200 && status < 400, blocked: status === 0 || status >= 400, bytes: respLen }), 10000);
            req.end();
          } catch { resolve({ kind: 'download', transferred: false, blocked: true, bytes: 0 }); }
        }));
      }

      const results = await Promise.all(promises);
      for (const r of results) dataExchanged += r.bytes;
      const uploads = results.filter(r => r.kind === 'upload');
      const downloads = results.filter(r => r.kind === 'download');
      const uploadsTransferred = uploads.filter(r => r.transferred).length;
      const uploadsBlocked = uploads.filter(r => !r.transferred).length;
      const downloadsTransferred = downloads.filter(r => r.transferred).length;
      const downloadsBlocked = downloads.filter(r => !r.transferred).length;

      logger.info(`[h2-fv] Virus multi-stream: uploads ${uploadsTransferred}/${uploads.length} transferred, ${uploadsBlocked} blocked; downloads ${downloadsTransferred}/${downloads.length} transferred, ${downloadsBlocked} blocked; ${dataExchanged} bytes exchanged`);
      return {
        status: 'PASSED',
        response: `${summarizeThreatTransfers({ protocol: 'HTTP/2', family: 'virus', total: downloads.length, transferred: downloadsTransferred, blocked: downloadsBlocked, bytes: dataExchanged })}; uploads ${uploadsTransferred}/${uploads.length} transferred, ${uploadsBlocked} blocked by FW`,
      };
    },
    expected: 'PASSED',
  },
  {
    ...
    clientHandler: async (session, host, logger) => {
      ...
      for (let i = 0; i < payloads.length; i++) {
        ...
        promises.push(new Promise((resolve) => {
          try {
            ...
            req.on('end', () => resolve({ kind: 'upload', transferred: status >= 200 && status < 400, blocked: status === 0 || status >= 400, bytes: body.length + respLen }));
            req.on('error', () => resolve({ kind: 'upload', transferred: false, blocked: true, bytes: 0 }));
            setTimeout(() => resolve({ kind: 'upload', transferred: status >= 200 && status < 400, blocked: status === 0 || status >= 400, bytes: body.length + respLen }), 15000);
            req.write(body);
            req.end();
          } catch { resolve({ kind: 'upload', transferred: false, blocked: true, bytes: 0 }); }
        }));
        ...
        promises.push(new Promise((resolve) => {
          try {
            ...
            req.on('end', () => resolve({ kind: 'download', transferred: status >= 200 && status < 400, blocked: status === 0 || status >= 400, bytes: respLen }));
            req.on('error', () => resolve({ kind: 'download', transferred: false, blocked: true, bytes: 0 }));
            setTimeout(() => resolve({ kind: 'download', transferred: status >= 200 && status < 400, blocked: status === 0 || status >= 400, bytes: respLen }), 15000);
            req.end();
          } catch { resolve({ kind: 'download', transferred: false, blocked: true, bytes: 0 }); }
        }));
      }
      ...
      logger.info(`[h2-fv] Sandbox multi-stream: uploads ${uploadsTransferred}/${uploads.length} transferred, ${uploadsBlocked} blocked; downloads ${downloadsTransferred}/${downloads.length} transferred, ${downloadsBlocked} blocked; ${dataExchanged} bytes exchanged`);
      return {
        status: 'PASSED',
        response: `${summarizeThreatTransfers({ protocol: 'HTTP/2', family: 'sandbox', total: downloads.length, transferred: downloadsTransferred, blocked: downloadsBlocked, bytes: dataExchanged })}; uploads ${uploadsTransferred}/${uploads.length} transferred, ${uploadsBlocked} blocked by FW`,
      };
    },
    expected: 'PASSED',
  },
  ...
  {
    name: 'h2-fv-max-streams-get-100kb',
    category: 'AM',
    description: 'Functional: HTTP/2 connection + 100 concurrent GET streams each downloading 100KB',
    side: 'client',
    useNodeH2: true,
    clientHandler: async (session, host, logger) => {
      const STREAM_COUNT = 100;
      const PAYLOAD_SIZE = 102400; // 100KB
      let succeeded = 0;
      let totalBytes = 0;
      let errors = 0;

      // Spread session.request() calls over event-loop ticks so nghttp2's
      // rapid-stream-creation flood defense doesn't RST tail streams with
      // ENHANCE_YOUR_CALM. setImmediate between requests is enough on localhost.
      const promises = [];
      for (let i = 0; i < STREAM_COUNT; i++) {
        promises.push(new Promise((resolve) => {
          try {
            const req = session.request({ ':method': 'GET', ':path': `/data?size=${PAYLOAD_SIZE}&stream=${i}`, ':scheme': 'https', ':authority': host });
            let bytes = 0;
            req.on('response', () => {});
            req.on('data', (d) => { bytes += d.length; totalBytes += d.length; });
            req.on('end', () => { succeeded++; resolve(bytes); });
            req.on('error', (e) => { errors++; logger.info(`Stream ${i} error: ${e.message}`); resolve(0); });
            setTimeout(() => resolve(bytes), 15000);
            req.end();
          } catch (e) { errors++; resolve(0); }
        }));
        await new Promise(r => setImmediate(r));
      }

      const bytesPerStream = await Promise.all(promises);
      if (succeeded === 0) {
        return { status: 'DROPPED', response: `All ${STREAM_COUNT} GET streams failed` };
      }
      if (errors > 0) return { status: errors === STREAM_COUNT ? 'DROPPED' : 'PASSED', response: `${succeeded}/${STREAM_COUNT} GET streams OK, ${errors} errors, ${totalBytes} bytes` };
      const allCorrect = bytesPerStream.every(b => b === PAYLOAD_SIZE);
      return {
        status: 'PASSED',
        response: `${succeeded}/${STREAM_COUNT} GET streams OK, ${totalBytes / 1024}KB total${allCorrect ? ', all 100KB verified' : ''}`,
      };
    },
    expected: 'PASSED',
  },
  ...
  {
    name: 'h2-fv-max-streams-post-100kb',
    category: 'AM',
    description: 'Functional: HTTP/2 connection + 100 concurrent POST streams each uploading and echo-verifying 100KB',
    side: 'client',
    useNodeH2: true,
    clientHandler: async (session, host, logger) => {
      const crypto = require('crypto');
      const STREAM_COUNT = 100;
      const PAYLOAD_SIZE = 102400; // 100KB
      const payloads = Array.from({ length: STREAM_COUNT }, () => crypto.randomBytes(PAYLOAD_SIZE));
      let succeeded = 0;
      let verified = 0;
      let errors = 0;
      ...
      for (let i = 0; i < STREAM_COUNT; i++) {
        ...
        promises.push(new Promise((resolve) => {
          try {
            const req = session.request({ ':method': 'POST', ':path': `/echo?stream=${i}`, ':scheme': 'https', ':authority': host, 'content-length': payload.length.toString() });
            let buf = Buffer.alloc(0);
            req.on('response', () => {});
            req.on('data', (d) => { buf = Buffer.concat([buf, d]); });
            req.on('end', () => {
              succeeded++;
              if (buf.equals(payload)) verified++;
              else logger.info(`Stream ${i}: echo mismatch (sent ${payload.length}B, got ${buf.length}B)`);
              resolve(buf.length);
            });
            req.on('error', (e) => { errors++; logger.info(`Stream ${i} error: ${e.message}`); resolve(0); });
            setTimeout(() => resolve(buf.length), 15000);
            req.write(payload);
            req.end();
          } catch (e) { errors++; resolve(0); }
        }));
      ...
      if (errors > 0) return { status: errors === STREAM_COUNT ? 'DROPPED' : 'PASSED', response: `${succeeded}/${STREAM_COUNT} POST streams OK, ${errors} errors` };
      if (verified < STREAM_COUNT) return { status: 'PASSED', response: `${succeeded}/${STREAM_COUNT} POST streams OK but only ${verified} echo-verified` };
      const totalKB = (STREAM_COUNT * PAYLOAD_SIZE * 2) / 1024;
      return { status: 'PASSED', response: `${verified}/${STREAM_COUNT} POST streams echo-verified, ${totalKB}KB exchanged` };
    },
    expected: 'PASSED',
  },

  {
    name: 'h2-fv-max-streams-delete-100kb',
    category: 'AM',
    description: 'Functional: HTTP/2 connection + 100 concurrent DELETE streams each receiving a 100KB response',
    side: 'client',
    useNodeH2: true,
    clientHandler: async (session, host, logger) => {
      const STREAM_COUNT = 100;
      const PAYLOAD_SIZE = 102400; // 100KB response per stream
      let succeeded = 0;
      let totalBytes = 0;
      let errors = 0;
      ...
      for (let i = 0; i < STREAM_COUNT; i++) {
        promises.push(new Promise((resolve) => {
          try {
            const req = session.request({ ':method': 'DELETE', ':path': `/resource/${i}?size=${PAYLOAD_SIZE}`, ':scheme': 'https', ':authority': host });
            let bytes = 0;
            req.on('response', () => {});
            req.on('data', (d) => { bytes += d.length; totalBytes += d.length; });
            req.on('end', () => { succeeded++; resolve(bytes); });
      ...
      await Promise.all(promises);
      if (errors > 0) return { status: errors === STREAM_COUNT ? 'DROPPED' : 'PASSED', response: `${succeeded}/${STREAM_COUNT} DELETE streams OK, ${errors} errors, ${totalBytes} bytes` };
      return { status: 'PASSED', response: `${succeeded}/${STREAM_COUNT} DELETE streams OK, ${totalBytes / 1024}KB received` };
    },
    expected: 'PASSED',
  },

  {
    ...
    side: 'server',
    multiStream: true,
    serverHandler: (stream, session, log, headers = {}) => {
      ...
      const url = (() => {
        try {
          ...
          return { pathname: path, searchParams: new Map() };
        }
      })();

      const respond = (body, extraHeaders = {}) => {
      ...
      try {
          ...
          stream.on('end', () => {
            try {
              ...
              if (url.pathname === '/echo') {
                log(`Echoing ${body.length}B for ${method} ${path}`);
                respond(body, { 'content-type': 'application/octet-stream' });
                return;
              }
              log(`Handling ${method} ${path} with ${body.length}B body`);
              respond('HTTP/2 OK');
            } catch (e) {
        ...
        if (url.pathname === '/data') {
          const size = Math.max(0, Math.min(102400, parseInt(url.searchParams.get('size') || '0', 10) || 0));
          log(`Serving ${size}B payload for ${path}`);
          respond(Buffer.alloc(size, 0x41), { 'content-type': 'application/octet-stream' });
          return;
        }

        if (url.pathname.startsWith('/virus-download/') || url.pathname.startsWith('/sb-download/')) {
          log(`Handling download ${path}`);
          respond('HTTP/2 OK');
          return;
        }

        log(`Handling ${method} ${path} with 200 OK (baseline)...`);
        respond('HTTP/2 OK');
      } catch (e) {
        log(`Response error: ${e.message}`);
      }
...
// PAN-SNI and PAN-PQC HTTP/2 scenarios removed — TLS-specific, not applicable to cleartext h2c
