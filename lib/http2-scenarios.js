...
const HTTP2_SCENARIOS = [
...

  {
    ...
    serverHandler: (stream, session, log) => {
      ...
      try { stream.respond({ ':status': 200 }); stream.end('push-promise-padded-truncated'); } catch (_) {}
    },
    expected: 'DROPPED',
...

  {
    ...
    serverHandler: (stream, session, log) => {
      ...
      try { stream.respond({ ':status': 200 }); stream.end('push-promise-padded-short-id'); } catch (_) {}
    },
    expected: 'DROPPED',
