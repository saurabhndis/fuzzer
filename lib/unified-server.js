...
// LDAP category codes start with 'L' followed by uppercase letter(s) (LA–LL)
function isLdapScenario(scenario) {
  return typeof scenario.category === 'string' && scenario.category.length === 2
    && scenario.category[0] === 'L' && scenario.category[1] >= 'A' && scenario.category[1] <= 'Z';
}

// H2 scenarios have a serverHandler function; TLS scenarios use actions()
// Must exclude LDAP server scenarios which also have serverHandler
function isH2Scenario(scenario) {
  return !isQuicScenario(scenario) && !isTcpScenario(scenario) && !isLdapScenario(scenario)
    && typeof scenario.serverHandler === 'function';
}
...
class UnifiedServer {
  constructor(opts = {}) {
    ...
    this.ldapServer = null;  // LdapFuzzerServer (persistent, shared across LDAP scenarios)
  ...
  abort() {
    ...
    if (this.ldapServer) { this.ldapServer.abort(); this.ldapServer = null; }
  ...
  async runScenario(scenario) {
    ...
    const result = await new Promise(async (resolve) => {
      ...
      try {
        let result;
        if (isLdapScenario(scenario)) result = await this._runLdapScenario(scenario);
        else if (isTcpScenario(scenario)) result = await this._runRawTCPScenario(scenario);
  ...
  runScenarioOnSocket(scenario, socket) {
    ...
    if (isLdapScenario(scenario)) return this._execLdapOnSocket(scenario, socket);
  ...
  // Execute an LDAP server scenario on a pre-connected raw TCP socket (multi-worker mode)
  _execLdapOnSocket(scenario, socket) {
    return new Promise((resolve) => {
      ...
      const done = (status, response) => {
        ...
        const verdict = (status === 'ERROR' || status === 'ABORTED' || status === 'SKIPPED') ? 'N/A'
          : (status === 'TIMEOUT' ? 'DROPPED' : status) === expected ? 'AS EXPECTED' : 'UNEXPECTED';
        resolve({
          scenario: scenario.name,
          description: scenario.description,
      ...
      const timer = setTimeout(() => done('TIMEOUT', 'No client data within timeout'), timeout);
      const log = (msg) => this.logger.info(`[${scenario.name}] ${msg}`);

      socket.on('data', (data) => {
        this.logger.received(data);
      });
      socket.on('end', () => this.logger.tcpEvent('received', 'FIN'));
      socket.on('close', () => { if (!resolved) done('PASSED', 'Client disconnected'); });
      socket.on('error', (err) => { if (!resolved) done('PASSED', `Client error: ${err.message}`); });

      // Wrap socket.write for logging
      ...
      try {
        ...
        if (handlerResult && typeof handlerResult.then === 'function') {
          handlerResult
            .then((res) => { if (res && typeof res === 'object' && res.status) done(res.status, res.response || ''); })
  ...
  async _runLdapScenario(scenario) {
    if (!this.ldapServer) {
      this.ldapServer = new LdapFuzzerServer({
        port: this.port,
        hostname: this.hostname,
        timeout: this.timeout,
        delay: this.delay,
    ...
    if (this.aborted) return { scenario: scenario.name, description: scenario.description, status: 'ABORTED', response: 'Aborted' };
    if (this._onListening) this._onListening();
    return this.ldapServer.runScenario(scenario);
  }
