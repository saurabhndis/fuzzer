...
function getOpenSSLGroups() {
  ...
  const addIfPresent = (out, name) => {
    if (out.includes(name) && !groups.includes(name)) groups.push(name);
  };

  try {
    const out = execSync('openssl list -tls-groups 2>/dev/null', { encoding: 'utf8', timeout: 5000 });
    addIfPresent(out, 'X25519MLKEM768');
    addIfPresent(out, 'SecP256r1MLKEM768');
    addIfPresent(out, 'SecP384r1MLKEM1024');
    addIfPresent(out, 'MLKEM768');
    addIfPresent(out, 'MLKEM1024');
    addIfPresent(out, 'MLKEM512');
  } catch (_) {
    try {
...
class UnifiedServer {
  ...
  _execNodeTLSOnSocket(scenario, rawSocket) {
    return new Promise((resolve) => {
      ...
      const pqcGroups = getOpenSSLGroups();
