  ...
  async function runScenarios() {
    ...
    if (role === 'client') {
      ...
      if (allWellBehaved) {
        ...
        const server = new UnifiedServer({ port, hostname, timeout, delay, logger, dut, pcapFile });
        state.fuzzer = server;
        
        const certInfo = server.getCertInfo();
        broadcastEvent({ type: 'logger', event: { type: 'info', ts: new Date().toISOString(), message: `Server certificate: CN=${hostname} | SHA256=${certInfo.fingerprint}` } });

        if (protocol === 'h2') await server.startH2();
        else if (protocol === 'quic') await server.startQuic();
        else await server.startTLS();

        // Mark all as passed immediately or as they come in? 
        // For well-behaved mode, the server just stays up. 
        // We report progress based on the client's progress.
        broadcastEvent({ type: 'status', role, status: 'running' });
        
        // In this mode, the server agent just "exists" until stopped.
        // We can't easily track individual results here because it's a generic responder.
        // But the Controller only cares about the client results anyway.
        return;
      }
