  ...
  function handleRunScenario(req, res) {
    ...
    req.on('end', async () => {
      try {
        ...
        // Helpers are renderer-assigned counterparts, never user-selected
        // fuzz targets. Their results go to the debug-result channel so the
        // main-process bridge can merge them into the paired fuzz row. The
        // patterns below cover every helper name the renderer generates
        // (well-behaved-*, srv-quic-…, fv-tls-…, and the SMTP/FTP/LDAP
        // STARTTLS pairs) but deliberately exclude diagnostic baselines like
        // `quic-well-behaved-single-get` so those still surface as visible
        // result rows.
        const helperName = result && result.scenario;
        const isHelperResult = !!helperName && (
