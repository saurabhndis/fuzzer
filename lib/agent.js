...
function startAgent(role, opts = {}) {
  ...
  const logger = {
    ...
    sent: (data, label) => broadcastEvent({ type: 'logger', event: { type: 'sent', ts: new Date().toISOString(), data: data.toString('hex'), label: label || '' } }),
    received: (data) => broadcastEvent({ type: 'logger', event: { type: 'received', ts: new Date().toISOString(), data: data.toString('hex') } }),
    tcpEvent: (dir, label) => broadcastEvent({ type: 'logger', event: { type: 'tcp-event', ts: new Date().toISOString(), dir, label } }),
    hostDown: (host, port, scenario) => broadcastEvent({ type: 'logger', event: { type: 'host-down', ts: new Date().toISOString(), host, port, scenario } }),
  ...
  async function runScenarios() {
    ...
    if (role === 'server') {
      ...
      const server = new UnifiedServer({ port, timeout, delay, logger });
      state.fuzzer = server;
