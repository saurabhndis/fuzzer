...
function startAgent(role, opts = {}) {
  ...
  async function runScenarios() {
    ...
    // HTTP/2 and QUIC are capped to 1 worker until concurrency issues are resolved.
    // TLS client scenarios (useNodeTLS) are safe to parallelize — each worker forks
    // its own process with an independent UnifiedClient and TLS connection.
    const isSingleWorkerProtocol = protocol === 'h2' || protocol === 'quic';
    const workers = (role === 'server' || isSingleWorkerProtocol) ? 1 : (parseInt(config.workers) || 1);
    const host = config.host;
    const port = config.port;
