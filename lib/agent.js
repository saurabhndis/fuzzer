...
function startAgent(role, opts = {}) {
  ...
  const logger = {
    ...
    barrierArrived: (scenario, label, role) => { broadcastEvent({ type: 'barrier-arrived', ts: new Date().toISOString(), scenario, label, role }); tr('barrier-arrived', { label, role }); },
    // Emitted when the fuzzer hits `{ type: 'peer-done' }`. The controller
    // reacts by POSTing /abort-scenario to the peer agent.
    peerDone: (scenario, role) => { broadcastEvent({ type: 'peer-done', ts: new Date().toISOString(), scenario, role }); tr('peer-done', { role }); },
  ...
  function handleReleaseBarrier(req, res) {
    ...
    req.on('end', () => {
      ...
      if (state.fuzzer && state.fuzzer.barrier) {
        const released = state.fuzzer.barrier.release(label, reason);
  ...
  async function ensureServerStarted() {
    ...
    if (!state.fuzzer) {
      state.fuzzer = new UnifiedServer({ port, hostname: config.hostname || config.host, bindAddress: config.bindAddress, timeout, delay, logger });
    }
  ...
  function handleRunScenario(req, res) {
    ...
    req.on('end', async () => {
      try {
        ...
        state.status = 'running';
        broadcastEvent({ type: 'progress', scenario: scenario.name, current: index + 1, total: state.scenarios.length });
        ...
        if (state.tracer) state.tracer.open(scenario.name);
        ...
        // Skip well-behaved partner results from the results array — they are internal helpers
        if (!(result.scenario && result.scenario.startsWith('well-behaved-')) &&
            !(result.scenario && result.scenario.startsWith('srv-quic-well-behaved-'))) {
