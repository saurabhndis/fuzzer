...
class UnifiedClient {
  constructor(opts = {}) {
    ...
    this._localPortMin = opts.localPortMin || 40000;
    this._localPortMax = opts.localPortMax || 60999;
    this._nextLocalPort = opts.localPortStart
      || (this._localPortMin + Math.floor(Math.random() * Math.max(1, this._localPortMax - this._localPortMin - 1024)));
    this._reservedLocalPorts = new Set();
  ...
  abort() {
    this.aborted = true;
    if (this._activeSubFuzzer && typeof this._activeSubFuzzer.abort === 'function') {
      try { this._activeSubFuzzer.abort(); } catch (_) {}
    }
  ...
  // Called by the agent's /abort-scenario handler when the controller
  // signals peer-done. Without this, QuicheClient/QuicFuzzerClient sit on
  // their own connect/handshake timeout (~5s per pair) regardless of the
  // server-side fuzz scenario finishing in milliseconds.
  ...
  _reserveLocalPort(proto = 'tcp') {
    const span = this._localPortMax - this._localPortMin + 1;
    for (let i = 0; i < span; i++) {
      const candidate = this._nextLocalPort;
      this._nextLocalPort++;
      if (this._nextLocalPort > this._localPortMax) this._nextLocalPort = this._localPortMin;
      if (this._reservedLocalPorts.has(candidate)) continue;
      this._reservedLocalPorts.add(candidate);
      this.logger.info(`[ports] reserved local ${proto.toUpperCase()} port ${candidate}`);
      return candidate;
    }
    throw new Error(`No free reserved local ${proto.toUpperCase()} ports available in ${this._localPortMin}-${this._localPortMax}`);
  }

  _releaseReservedLocalPort(port, proto = 'tcp') {
    if (!port) return;
    if (this._reservedLocalPorts.delete(port)) {
      this.logger.info(`[ports] released unused local ${proto.toUpperCase()} port ${port}`);
    }
  }

  _logActionBreadcrumb(payload, { updateCurrent = true } = {}) {
    const record = {
      ...(payload || {}),
      localPort: payload && payload.localPort !== undefined ? payload.localPort : this._lastClientPort,
      updatedAt: new Date().toISOString(),
    };
    if (updateCurrent) this._currentAction = record;
    this._lastAction = record;
    if (this.logger && typeof this.logger.action === 'function') {
      try { this.logger.action(record); } catch (_) {}
    }
    return record;
  }

  _clearActionBreadcrumb() {
    this._currentAction = null;
  }
  ...
  getDebugState() {
    return {
      ...
      lastClientPort: this._lastClientPort,
      activeSocketCount: this.activeSockets.size,
      activeChildProcessCount: this.activeChildProcesses.size,
      reservedLocalPorts: Array.from(this._reservedLocalPorts).slice(0, 32),
    };
  }
  ...
  async runScenario(scenario) {
    ...
    return new Promise((resolve) => {
      ...
      const heartbeat = setInterval(() => {
        if (!finished && this._currentAction) {
          this._logActionBreadcrumb({
            ...this._currentAction,
            phase: 'heartbeat',
    ...
    try {
      ...
      for (let i = 0; i < actions.length; i++) {
        ...
        const actionMeta = {
          ...
          type: action.type,
          label: action.label || '',
          pairId: this._distributedPairId || null,
          runId: this._distributedRunId !== undefined ? this._distributedRunId : null,
        };
        let actionOutcome = 'completed';
        let stopAfterAction = false;
        this._logActionBreadcrumb({ ...actionMeta, phase: 'start', connectionClosed });
        ...
        switch (action.type) {
          case 'barrier': {
            try {
              this._logActionBreadcrumb({ ...actionMeta, phase: 'wait', waitFor: 'barrier', timeoutMs: action.timeout || 10000, connectionClosed });
          ...
          case 'peer-done': {
            ...
            actionOutcome = 'peer-done';
            break;
          }

          case 'send': {
            if (connectionClosed || socket.destroyed) {
              this.logger.error('Cannot send: connection closed'); status = 'DROPPED'; actionOutcome = 'send-closed'; break;
            }
            ...
            } catch (e) { this.logger.error(`Write failed: ${e.message}`); status = 'DROPPED'; actionOutcome = 'send-error'; }
            break;
          }
          ...
          case 'recv': {
            ...
            this._logActionBreadcrumb({ ...actionMeta, phase: 'wait', waitFor: 'recv', timeoutMs: action.timeout || this.timeout, connectionClosed });
            if (action.preserveAcceptSignal && action.stopAfterAcceptSignal) {
              await this._waitForBufferedTLSBurst(socket, action.timeout || this.timeout, () => connectionClosed, () => recvBuffer);
            } else {
            ...
            if (data && data.length > 0) {
              ...
              const describedResponse = this._describeTLSResponse(data);
              const acceptSignal = this._extractTLSServerAcceptance(data);
              if (acceptSignal && !firstMeaningfulResponse) {
                firstMeaningfulResponse = acceptSignal;
              }
              if (action.preserveAcceptSignal && acceptSignal) {
                const laterAlert = /^Alert\(/i.test(describedResponse) || /Encrypted alert/i.test(describedResponse);
                lastResponse = (laterAlert && !action.stopAfterAcceptSignal)
                  ? `${acceptSignal}; later ${describedResponse}`
                  : acceptSignal;
                if (action.statusOnAcceptSignal) status = action.statusOnAcceptSignal;
                if (action.stopAfterAcceptSignal) {
                  stopAfterAction = true;
                  actionOutcome = 'accept-signal';
                }
              } else {
                lastResponse = describedResponse;
              }
              rawResponse = data;
              if (hasFuzzAction) postFuzzRecvData = data;

              // If we received a cleartext alert, update status immediately
              if (/^Alert\(/i.test(describedResponse) && !(action.preserveAcceptSignal && acceptSignal && action.statusOnAcceptSignal === 'PASSED')) {
                status = 'tls-alert-server';
                actionOutcome = 'alert';
              }
            } else if (connectionClosed) {
              if (action.allowClose) {
                status = action.statusOnClose || status || 'PASSED';
                lastResponse = action.responseOnClose || 'Connection closed';
                rawResponse = null;
                actionOutcome = 'allow-close';
                break;
              }
          ...
          case 'tlsPost': {
            ...
            try {
              this._logActionBreadcrumb({ ...actionMeta, phase: 'wait', waitFor: 'tls-post', timeoutMs: this.timeout, connectionClosed });
          ...
          case 'tls12Handshake': {
            ...
            if (connectionClosed || socket.destroyed) {
              ...
              status = 'DROPPED'; actionOutcome = 'tls12-closed'; break;
            }
            try {
              const tls12 = require('./tls12-crypto');
              this.logger.info(action.label || 'TLS 1.2 ECDHE Handshake');
              this._logActionBreadcrumb({ ...actionMeta, phase: 'wait', waitFor: 'tls12-handshake', timeoutMs: action.timeout || this.timeout, connectionClosed });
          ...
          this._logActionBreadcrumb({
            ...
            status,
            connectionClosed: connectionClosed || !!(socket && socket.destroyed),
      ...
      if (!lastResponse && firstMeaningfulResponse) {
        lastResponse = firstMeaningfulResponse;
      }
    } catch (e) {
  ...
  async _runNodeTLSClient(scenario) {
    ...
    try {
      // When PCAP is enabled, route through a local TCP proxy to capture the real
      // encrypted TLS bytes on the wire. Node's tls module does socket I/O in C++
      // that bypasses JS-level write/'data' hooks, so a relay proxy is the only
      // reliable capture point (same approach as _runNodeH2Client). The companion
      // .keylog lets Wireshark decrypt the captured records.
      const savedHost = this.host;
      const savedPort = this.port;
      if (this.pcap) {
        pcapProxy = await this._startPcapProxy();
        this.host = '127.0.0.1';
        this.port = pcapProxy.port;
        this.pcap.writeTCPHandshake();
      }
      try {
        socket = await this._connectNodeTLSWithRetry(scenario.nodeTlsOptions || {});
      } finally {
        // Restore original host/port before running the handler so Host headers,
        // SNI-derived authority, and logging reflect the real target.
        this.host = savedHost;
        this.port = savedPort;
      }
      ...
      if (scenario.clientHandler) {
        ...
        socket.write(req);
        if (this.pcap && !pcapProxy) this.pcap.writeTLSData(Buffer.from(req), 'sent');
        ...
        const data = await new Promise((resolve) => {
          ...
          socket.on('data', (d) => {
            ...
            if (this.pcap && !pcapProxy) this.pcap.writeTLSData(d, 'received');
  ...
  async _connectNodeH2WithRetry(options = {}) {
    ...
    for (let attempt = 0; attempt <= maxRetries; attempt++) {
      let session = null;
      let reservedLocalPort = null;
      try {
        return await new Promise((resolve, reject) => {
          let settled = false;
          const connOpts = { ...options };
          let connected = false;
          reservedLocalPort = this._reserveLocalPort('tcp');
          connOpts.createConnection = () => net.connect({
            host: this.host,
            port: this.port,
            localPort: reservedLocalPort,
          });
          ...
          session.on('connect', (sess, sock) => {
            ...
            settled = true;
            clearTimeout(timer);
            const liveSocket = sock || session.socket;
            if (liveSocket) {
              this._lastClientPort = liveSocket.localPort;
              this.logger.info(`[node-h2] Connected from local port ${liveSocket.localPort}`);
            }
            resolve(session);
          });
          session.on('error', (err) => {
            if (settled) return;
            if (!connected) this._releaseReservedLocalPort(reservedLocalPort, 'tcp');
            settled = true;
            clearTimeout(timer);
            reject(err);
          });
          const timer = setTimeout(() => {
            ...
            if (!connected) this._releaseReservedLocalPort(reservedLocalPort, 'tcp');
            settled = true;
            // close() flushes nghttp2 state before destroy() frees memory; required to
            // avoid the ~Http2Session(current_nghttp2_memory_ == 0) assertion at GC.
            try { session.close(); } catch (_) {}
            try { session.destroy(); } catch (_) {}
        ...
        this._releaseReservedLocalPort(reservedLocalPort, 'tcp');
      ...
      for (const action of actions) {
        ...
        switch (action.type) {
          ...
          case 'recv': {
            ...
            recvBuffer = Buffer.alloc(0);
            const data = Buffer.concat([alreadyReceived, tailBuffer]);
  ...
  _connectTLSOnce() {
    return new Promise((resolve, reject) => {
      ...
      const localPort = this._reserveLocalPort('tcp');
      let connected = false;
      const socket = net.createConnection({ host: this.host, port: this.port, allowHalfOpen: true, localPort }, () => {
        connected = true;
        ...
        this.logger.info(`Connected to ${this.host}:${this.port} from local port ${socket.localPort}`);
        this.activeSockets.add(socket);
        resolve(socket);
      });
      socket.setTimeout(this.timeout);
      socket.on('timeout', () => {
        if (!connected) this._releaseReservedLocalPort(localPort, 'tcp');
        socket.destroy();
  ...
  _connectH2Once(connectionOptions, isTcpOnly) {
    ...
    return new Promise((resolve, reject) => {
      const localPort = this._reserveLocalPort('tcp');
      let connected = false;
      const socket = net.createConnection({ host: this.host, port: this.port, localPort });
      this.activeSockets.add(socket);
      socket.setTimeout(this.timeout);
      ...
      socket.on('connect', () => {
        ...
        this.logger.info(`TCP connected to ${this.host}:${this.port} from local port ${socket.localPort} (h2c cleartext)`);
        resolve(socket);
      });
  ...
  _runTLSPost(action) {
    ...
    return new Promise((resolve, reject) => {
      ...
      tlsSocket.on('secureConnect', () => {
        connected = true;
        this._lastClientPort = tlsSocket.localPort;
        this.logger.info(`TLS app connection from local port ${tlsSocket.localPort}`);
  ...
  _extractTLSServerAcceptance(data) {
    ...
    for (const r of records) {
      if (r.type !== ContentType.HANDSHAKE || r.payload.length < 40) continue;
      const hsType = r.payload[0];
      if (hsType !== HandshakeType.SERVER_HELLO) continue;
      const { getServerHelloVersion } = require('./constants');
      ...
      const random = r.payload.subarray(6, 38);
      if (random.length === HELLO_RETRY_RANDOM.length && random.equals(HELLO_RETRY_RANDOM)) {
        if (csOffset + 1 < r.payload.length) {
          ...
          return `HelloRetryRequest(${vName}, ${csName})`;
        }
        return `HelloRetryRequest(${vName})`;
      }
      if (csOffset + 1 < r.payload.length) {
        const cs = (r.payload[csOffset] << 8) | r.payload[csOffset + 1];
        const csName = CipherSuiteName[cs] || `0x${cs.toString(16)}`;
  ...
  _computeVerdict(status, expected, response) {
    ...
    if (expected === 'TIMEOUT') return status === 'TIMEOUT' ? 'AS EXPECTED' : 'UNEXPECTED';
  ...
  _waitForBufferedTLSBurst(socket, timeout, isClosedFn, getBuffer) {
    return new Promise((resolve) => {
      ...
      const scheduleSettle = () => {
        if (settleTimer) clearTimeout(settleTimer);
        settleTimer = setTimeout(done, 75);
      };
  ...
  async _runQuicScenario(scenario) {
    ...
    if (scenario.useQuiche) {
      ...
      const quicheClient = new QuicheClient({
        ...
        protocol: 'udp',
        localPortAllocator: () => this._reserveLocalPort('udp'),
        releaseLocalPort: (port) => this._releaseReservedLocalPort(port, 'udp'),
      });
      // Track the active sub-fuzzer so abort()/cancelScenarioWaits() can
      // propagate down. If the agent already flipped _scenarioAborted while
      // we were spinning up, abort the sub-fuzzer immediately.
      this._activeSubFuzzer = quicheClient;
      if (this._scenarioAborted || this.aborted) {
        try { quicheClient.abort(); } catch (_) {}
      }
      try {
        return await quicheClient.runScenario(scenario);
      } finally {
        if (this._activeSubFuzzer === quicheClient) this._activeSubFuzzer = null;
      }
    }
    ...
    this._activeSubFuzzer = quicClient;
    if (this._scenarioAborted || this.aborted) {
      try { quicClient.abort(); } catch (_) {}
    }
    try {
      return await quicClient.runScenario(scenario);
    } finally {
      if (this._activeSubFuzzer === quicClient) this._activeSubFuzzer = null;
    }
  }
  ...
  async _connectNodeTLSWithRetry(options = {}) {
    ...
    const { clientHelloFragment, ...tlsOptions } = options || {};
    const maxRetries = 70;
    const retryDelay = 500;
    for (let attempt = 0; attempt <= maxRetries; attempt++) {
      let sock = null;
      let rawSocket = null;
      let reservedLocalPort = null;
      let fragmentProxy = null;
      try {
        reservedLocalPort = this._reserveLocalPort('tcp');
        if (clientHelloFragment) {
          fragmentProxy = await this._startClientHelloFragmentProxy(clientHelloFragment, reservedLocalPort);
        }
        ...
        return await new Promise((resolve, reject) => {
          ...
          this._nodeTlsClientHelloFragmented = false;
          rawSocket.on('data', (d) => {
            if (this.pcap) this.pcap.writeTLSData(d, 'received');
          });
          const origWrite = rawSocket.write.bind(rawSocket);
          rawSocket.write = (data, encoding, callback) => {
            if (typeof encoding === 'function') {
              callback = encoding;
              encoding = undefined;
            }
            const chunks = this._fragmentClientHelloWrite(data, fragmentProxy ? null : clientHelloFragment, encoding);
            let ret = true;
            for (let i = 0; i < chunks.length; i++) {
              const chunk = chunks[i];
              if (this.pcap && !fragmentProxy) this.pcap.writeTLSData(chunk, 'sent');
              ret = origWrite(chunk, i === chunks.length - 1 ? encoding : undefined, i === chunks.length - 1 ? callback : undefined);
            }
            return ret;
          };

          const connectOpts = fragmentProxy
            ? { host: '127.0.0.1', port: fragmentProxy.port }
            : { host: this.host, port: this.port, localPort: reservedLocalPort };

          rawSocket.connect(connectOpts, () => {
             sock = tls.connect({
                socket: rawSocket,
                host: this.host,
                rejectUnauthorized: false,
                ...
                settled = true;
                clearTimeout(timer);
                this._lastClientPort = fragmentProxy && fragmentProxy.state.outboundLocalPort
                  ? fragmentProxy.state.outboundLocalPort
                  : sock.localPort;
                this.logger.info(`[node-tls] Connected from local port ${this._lastClientPort}`);
                resolve(sock);
              });
        ...
        if (fragmentProxy && fragmentProxy.server) {
          try { fragmentProxy.server.close(); } catch (_) {}
        }
        this._releaseReservedLocalPort(reservedLocalPort, 'tcp');
  ...
  _fragmentClientHelloWrite(data, fragment, encoding) {
    if (!fragment || this._nodeTlsClientHelloFragmented) {
      return [Buffer.isBuffer(data) ? data : Buffer.from(data, encoding)];
    }

    const input = Buffer.isBuffer(data) ? data : Buffer.from(data, encoding);
    const hostname = Buffer.from(fragment.hostname || '', 'ascii');
    if (hostname.length === 0 || input.length < 5) return [input];

    let off = 0;
    const out = [];
    while (off + 5 <= input.length) {
      const type = input[off];
      const version = input.subarray(off + 1, off + 3);
      const len = input.readUInt16BE(off + 3);
      const recordEnd = off + 5 + len;
      if (recordEnd > input.length) return [input];

      const payload = input.subarray(off + 5, recordEnd);
      if (type === ContentType.HANDSHAKE && payload.length >= 4 && payload[0] === HandshakeType.CLIENT_HELLO) {
        const hostOffset = payload.indexOf(hostname);
        if (hostOffset !== -1) {
          const mode = fragment.mode || 'split';
          const splitPoint = mode === 'delayed'
            ? hostOffset
            : hostOffset + Math.max(1, Math.floor(hostname.length / 2));
          if (splitPoint > 0 && splitPoint < payload.length) {
            out.push(input.subarray(0, off));
            out.push(this._buildTLSRecord(type, version, payload.subarray(0, splitPoint)));
            out.push(this._buildTLSRecord(type, version, payload.subarray(splitPoint)));
            out.push(input.subarray(recordEnd));
            this._nodeTlsClientHelloFragmented = true;
            this.logger.info(`[node-tls] Fragmented ClientHello at SNI (${mode}, split=${splitPoint}/${payload.length})`);
  ...
  _buildTLSRecord(type, versionBytes, payload) {
    const header = Buffer.alloc(5);
    header[0] = type;
    versionBytes.copy(header, 1);
    header.writeUInt16BE(payload.length, 3);
    return Buffer.concat([header, payload]);
  }

  _startClientHelloFragmentProxy(fragment, outboundLocalPort) {
    return new Promise((resolve, reject) => {
      const state = { outboundLocalPort: null };
      const targetHost = this.host;
      const targetPort = this.port;
      const server = net.createServer((clientSock) => {
        ...
        const serverSock = net.connect({
          host: targetHost,
          port: targetPort,
          localPort: outboundLocalPort,
        });

        const closeBoth = () => {
          try { if (!clientSock.destroyed) clientSock.destroy(); } catch (_) {}
          try { if (!serverSock.destroyed) serverSock.destroy(); } catch (_) {}
        };

        serverSock.on('connect', () => {
          state.outboundLocalPort = serverSock.localPort;
        });
        clientSock.on('data', (chunk) => {
          const chunks = this._fragmentClientHelloWrite(chunk, fragment);
          for (const out of chunks) {
            if (this.pcap) this.pcap.writeTLSData(out, 'sent');
            serverSock.write(out);
          }
        });
        ...
        serverSock.on('error', closeBoth);
        clientSock.on('close', closeBoth);
        serverSock.on('close', closeBoth);
      });
