      ...
      for (const action of actions) {
        ...
        switch (action.type) {
          ...
          case 'recv': {
            ...
            // Capture any late-arriving data that landed in recvBuffer after
            // _waitForData removed its listener (e.g. TLS alert after FIN)
            const lateData = recvBuffer;
            recvBuffer = Buffer.alloc(0);
            const data = Buffer.concat([alreadyReceived, dataFromWait || Buffer.alloc(0), lateData]);
