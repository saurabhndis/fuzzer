...
class UnifiedClient {
  ...
  async _runTLSScenario(scenario) {
    ...
    const actions = actionFn({ hostname: this.host });
    let socket = null;
        ...
        switch (action.type) {
          ...
          case 'tls12Handshake': {
            ...
            try {
              ...
              socket.write(action.clientHello);
              this.logger.sent(action.clientHello, 'ClientHello (PCAP verbatim)');
              if (this.pcap) this.pcap.writeTLSData(action.clientHello, 'sent');
              ...
              while (Date.now() - flightStart < flightTimeout && !connectionClosed && !gotServerHelloDone && !serverUsedTLS13) {
                ...
                recvBuffer = Buffer.alloc(0);
                const chunk = await this._waitForData(socket, Math.max(1000, flightTimeout - (Date.now() - flightStart)), () => connectionClosed);
                recvBuffer = Buffer.alloc(0);
              ...
              try {
                const { messages: chMsgs } = parseHsMsg(action.clientHello);
              ...
              const clientRandom = action.clientRandom || Buffer.alloc(32); // from PCAP
              const result = tls12.completeHandshake({
                clientHello: action.clientHello,
    ...
    for (let attempt = 0; attempt <= maxRetries; attempt++) {
      ...
      try {
        return await new Promise((resolve, reject) => {
          let settled = false;
          // Use createConnection to get the TLS socket before the handshake
          // fires, so keylog events are captured from the very start.
          const self = this;
          const connOpts = {
            ...
            createConnection(url, opts) {
              ...
              self._attachKeylog(sock);
              return sock;
            },
          };
          session = http2.connect(`https://${this.host}:${this.port}`, connOpts);
