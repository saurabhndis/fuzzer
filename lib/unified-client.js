    ...
    for (let attempt = 0; attempt <= maxRetries; attempt++) {
      ...
      try {
        return await new Promise((resolve, reject) => {
          ...
          rawSocket.connect({ host: this.host, port: this.port, localPort: reservedLocalPort }, () => {
             sock = tls.connect({
                socket: rawSocket,
                host: this.host,
                rejectUnauthorized: false,
                ...
                settled = true;
                clearTimeout(timer);
                this._lastClientPort = sock.localPort;
                this.logger.info(`[node-tls] Connected from local port ${sock.localPort}`);
                resolve(sock);
              });
