...
// Must exclude QUIC (Q*), TCP (R*), and LDAP (L*) two-letter categories
function isH2Scenario(scenario) {
  return typeof scenario.category === 'string' && scenario.category.length === 2
    && !isQuicScenario(scenario) && !isTcpScenario(scenario) && !isLdapScenario(scenario);
}
...
class UnifiedClient {
  ...
  async runScenario(scenario) {
    ...
    return new Promise(async (resolve) => {
      ...
      try {
        let result;
        if (isLdapScenario(scenario)) {
          result = await this._runLdapScenario(scenario);
        } else if (isTcpScenario(scenario)) {
  ...
  async _runLdapScenario(scenario) {
    const ldapClient = new LdapFuzzerClient({
      host: this.host,
      port: this.port,
    ...
    try {
      return await ldapClient.runScenario(scenario);
    } finally {
      ldapClient.close();
    }
  }
