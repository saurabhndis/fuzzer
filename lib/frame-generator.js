...
function writeRawFrame(session, frame) {
  const socket = session._tlsSocket || session._rawSocket || session.socket;
  if (socket && !socket.destroyed) {
    console.log(`\n--- Sending Frame (${frame.length} bytes) ---`);
    console.log(frame.toString('hex').match(/.{1,2}/g).join(' '));
    console.log('-----------------------------\n');
    socket.write(frame);
  }
}
...
module.exports = {
  ...
  buildFrame, writeRawFrame, writeRawFrameTLS,
};
